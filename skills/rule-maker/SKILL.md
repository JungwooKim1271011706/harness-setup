...
## ⚠ 정량 서술은 실측으로 센다 (rule 파일은 SSOT로 취급된다)

rule 파일은 하네스가 **SSOT로 취급**한다 — planner·developer·tester 전원이 Read하고 준수한다. 그런데 **게이트는 rule을 기준으로 삼지 rule을 의심하지 않는다.** 여기 들어간 사실 오류는 어느 게이트에도 안 걸리고 무기한 전파된다.

- **"테스트 N개" 류 정량 서술은 파일 개수가 아니라 `@Test`(또는 해당 프레임워크의 케이스 애노테이션) 개수로 센다.** 파일명이 `*Test`라도 `public static void main()` 스크래치일 수 있다.
- **서술에 근거를 병기한다** — 예: *"테스트 파일 2개 / 실제 `@Test` 0건 — 전부 `main()` 수동 스크래치"*.
- 같은 원칙을 **개수·비율·"~만 있다"·"~는 없다"** 류 모든 단언에 적용한다. 세지 않았으면 쓰지 않는다.

> 근거: rule의 *"테스트가 2개뿐"*이 실측 `@Test` **0건**이었다. 이 서술이 계획서 회귀범위표로, 다시 `-Dtest=` 목록으로 전파돼 surefire 무음 스킵을 낳았다 — "테스트 2개"와 "테스트 0개"는 **회귀 전략이 갈리는 차이**다(2026-09-06).
