...
# 소스 경로 매핑 (글로벌 → 로컬) — 자체/비-gstack 스킬만 동기화한다.
#
# ⚠ gstack 제공 스킬(office-hours·investigate·review·cso·benchmark·codex·browse·
#   plan-*-review·context-save/restore 등)은 더 이상 vendoring하지 않는다(v2.1.0).
#   글로벌 gstack 설치(~/.claude/skills/gstack/)에 의존하며, 미설치 시 session-check.sh가
#   설치를 안내한다. gstack 갱신은 `gstack-upgrade`로. → 여기서 동기화하지 않음.
#
# 아래는 gstack 비제공 자체 스킬. 원본 부재 시 sync는 "소스 없음"으로 스킵(repo 미러로 동작 유지).
declare -A SOURCES
