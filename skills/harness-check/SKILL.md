...
- 이 inbox 파일은 이후 `/harness-retro`가 드레인할 때 **`wiki/sources/`로 복사돼 wiki 페이지의 `sources` 근거 원문이 된다**(경로 인용이 아니라 사본 — inbox는 머신로컬·이동성 파일이라 경로가 죽는다. `wiki/_schema.md` "근거 고정"). 따라서 이 파일 **본문 자체가 근거로 읽힐 수 있어야** 한다 — 증상/추정원인/제안/근거/관련파일/발생세션 보존 필수.
- 두 repo(harness-setup·제품 gitlab) 어디도 안 건드린다 — inbox는 중립지대. 후보 0건이면 드롭도 생략.

## Step 3 — /harness-retro에 위임 (또는 inbox 안내)
- **dev clone 세션이면**(판별식 = `wiki/_schema.md` "어디로 가나" SSOT — **3단계·순서 필수**, `ROOT=$(git rev-parse --show-toplevel)`: ① basename `.claude` → 소비자 ② `$ROOT/.claude/.git` 존재 → 소비자 ③ `$ROOT/agents/orchestrator.md`+`VERSION` → dev clone. ⚠ **①만 떼어 쓰면 소비자(cwd=제품 repo 루트)를 dev clone으로 오판**한다 — 종전 문면이 그랬다(2026-08-04 정정). origin 판별도 금지): 변환 후보로 바로 **harness-retro 절차를 실행**한다(분류·라우팅·bump추론·초안·승인요청 = `skills/harness-retro/SKILL.md` SSOT, 중복 재구현 금지). ⚠ dev clone은 `/harness-retro` 슬래시 미등록(harness가 repo 루트라 `.claude/skills/` 경로 없음) → **파일 절차로 실행**, 슬래시 호출 아님.
