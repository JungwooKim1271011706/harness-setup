...
```bash
grep -rn "기계 강제가 없\|기계 강제 없\|장치가 없다\|프롬프트 룰뿐\|자동 탐지\|소프트 규칙\|산문 규칙\|재량 지시\|서브에이전트 재량\|탐지 수단이\|그물이 어디에도\|의심하지 않는다\|유일한 확인선\|흔적으로만" \
  agents/ docs/ --include="*.md" | grep -v "harness-retro"
```

⚠ **`skills/`는 대상에서 뺀다.** 이 SKILL.md 자신이 위 패턴을 품고 있어 **자기매치**가 섞인다 — 첫 실행(2026-09-09)에서 3건이 잡혔는데 그중 **2건이 이 파일의 grep 라인 자체**였다(실질 1건). 패턴도 그때 너무 좁아서 8건 중 1건만 걸렸다.

규칙을 쓸 때마다 *"기계 강제 없음"* 계열 문면이 남으므로 **이 목록은 저절로 갱신된다.** 손으로 관리하는 위시리스트는 반드시 stale해진다.

⚠ **단, 문면에 안 적힌 자리는 영영 안 잡힌다.** 탐색 파일 수 상한·스크린샷 육안 판정이 그랬다 — 강제 수단이 없는데 그 사실을 규칙 옆에 안 써서 grep 밖에 있었다. 그래서 Step 5에 **"강제 수단 없는 규칙은 그 사실을 문면에 남긴다"**를 짝으로 둔다. 추출이 성립하려면 **생산 쪽 습관이 전제**다.
...
- **강제 수단 없이 쓰는 규칙은 그 사실을 문면에 남긴다 (Step 2.5의 짝).** 훅·스크립트로 못 막고 프롬프트 준수에만 기대는 규칙이면, 그 자리에 *"기계 강제가 없으므로 <무엇>으로만 확인된다"* 또는 *"…를 자동 탐지하는 장치가 없다"*를 **1줄 덧댄다**. 이유 두 가지: ① 읽는 에이전트가 **그 규칙의 신뢰 수준을 안다**(자기보고에 기대는 축임을 알고 흔적을 남기게 된다) ② **Step 2.5가 grep으로 이 문면을 수집**한다 — 안 적으면 클코가 나중에 그걸 덮을 수단을 내놔도 **영영 후보에 안 오른다**. 실측: 탐색 파일 수 상한·스크린샷 육안 판정이 정확히 그래서 첫 실행 목록에서 빠졌다(2026-09-09).
