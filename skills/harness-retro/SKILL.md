...
- ⚠ **적용은 dev clone에서만**. 소비자 세션(제품 repo에 vendoring된 중첩 `.claude`)은 거기서 커밋하면 제품 repo에 갇히거나 SSOT와 갈린다 — 거기선 check 드롭까지만, 적용은 dev clone에서. **판별식 = `wiki/_schema.md` "어디로 가나" SSOT** — 3단계·순서 필수(`ROOT=$(git rev-parse --show-toplevel)`): ① basename `.claude` → 소비자 ② `$ROOT/.claude/.git` 존재 → 소비자 ③ `$ROOT/agents/orchestrator.md`+`VERSION` → dev clone. ⚠ **①만 떼어 쓰면 소비자(cwd=제품 repo 루트)를 dev clone으로 오판**하고[2026-08-04 정정], 방향을 뒤집으면 진짜 dev clone이 죽는다. origin 판별도 금지 — 중첩 `.claude`도 harness-setup이라 오판[2026-07-15 실사고].
...
## Step 2.5 — 클코 기능 대체 점검 (조건부, 드레인당 1회)

하네스에는 *"규칙은 있는데 기계 강제 수단이 없다"*고 문면에 적어둔 자리가 여럿 있다(탐색 한도, 참조 Read 흔적, 육안 판정, rule 사실 검증 등). Claude Code가 갱신되면 그중 일부를 **훅 이벤트·설정·도구로 덮을 수 있게 된다.** 이 Step은 그걸 **회고를 도는 김에** 한 번 확인한다 — 별도 스케줄·크론을 두지 않는다(뉴스를 상시 추적하면 소음이 규칙보다 커진다).

**발동 판정 (건너뛰기 우선)**

```bash
claude --version            # 예: 2.1.266 (Claude Code)
grep -m1 "클코 기능 대체 점검" CHANGELOG.md
```

CHANGELOG의 마지막 점검 줄에 적힌 버전과 **현재 버전이 같으면 이 Step을 건너뛴다**(그 버전에서 이미 물어봤다). 그 줄이 곧 마커다 — 별도 state 파일을 두지 않는다.

**입력 추출 — 유지할 목록을 따로 만들지 않는다**

```bash
grep -rn "기계 강제가 없\|기계 강제 없\|장치가 없다\|프롬프트 룰뿐\|자동 탐지하는 장치" \
  agents/ docs/ skills/ --include="*.md"
```

규칙을 쓸 때마다 *"기계 강제 없음"*이 문면에 남으므로 **이 목록은 저절로 갱신된다.** 손으로 관리하는 위시리스트는 반드시 stale해진다.

**질의 — 무엇을 묻느냐가 이 Step의 전부다**

`claude-code-guide` 서브에이전트에 **1회** 위임한다(WebFetch로 최신 문서를 본다). 프롬프트에 위 grep 결과를 그대로 싣고:

> ❌ *"새 기능 뭐 나왔나"* — 소음이다. 대부분 이 하네스와 무관하다.
> ✅ **"아래 N개 자리를 현재 Claude Code 기능(훅 이벤트·settings·도구·에이전트 정의)으로 기계 강제할 수 있는가? 덮을 수 있는 것만 답하고, 각각 어떤 수단인지 문서 근거를 대라. 없으면 '없음' 한 줄."**
> ✅ 역방향도 함께: **"하네스가 우회책으로 만든 규칙 중, 이제 클코 기능이 직접 해결해서 불필요해진 것이 있는가?"**(예: 병렬 발사 누락을 *선언 금지*로 우회한 것)

**출력**

- 덮을 수 있는 게 있으면 → **Step 4 백로그에 항목으로 기재**하고 Step 6 승인 게이트로 올린다. ⚠ **자동 적용 금지** — 훅 신설은 거버넌스 영향(MAJOR 후보)이다.
- 없으면 → 이번 드레인 CHANGELOG 항목에 **`클코 기능 대체 점검: 해당 없음 (클코 <버전>)`** 1줄. 이게 다음 드레인의 건너뛰기 근거가 된다.
- ⚠ 서브에이전트가 죽거나 WebFetch가 막히면 **`클코 기능 대체 점검: 미실행(사유)`**로 남긴다 — 조용히 생략하면 다음 드레인이 "이미 봤다"로 오인한다.
