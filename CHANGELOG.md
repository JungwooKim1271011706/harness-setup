...
> **클코 기능 대체 점검: 후보 2건 (클코 2.1.270)** — 백로그 `2.5-A`/`2.5-B`에 draft 기재, 훅 신설은 거버넌스라 미적용. 다음 드레인은 이 버전과 같으면 건너뛴다.

inbox 드레인 3파일(khnp WI 13 트랙3 / WI 12 주석정리, 2026-09-11) → **12건 적용 + 1건 reject**. 같은 결함 클래스 2쌍은 통합 처리했다.

- **[차단 훅이 읽기전용 grep을 2회 막았다 — 인용된 정규식 속 `|`]** `block-orchestrator-exec.sh`가 따옴표 **문자만** 떼는 탓에 `grep -E 'a|mvn install'` → `a|mvn install` → `|mvn ` 이 경계2의 *명령 위치*로 매치됐다. 경계2 주석은 *"인자 내부 mvn은 오탐 안 함"*이라 적었지만 **인자 안에 `|`가 있는 경우**를 못 막았다 — 경계2·경계3에 이은 **새 변형**이다.
  - 수정: `tr -d` → **따옴표 상태 추적 awk**. 인용 **밖** 제어연산자는 그대로 두고 **인용 안 `;&|()` 만 공백화**한다. 인용 안 *단어*는 보존하므로 `"git" commit` 우회 차단(경계1)은 유지.
  - 회귀 **17/17**: 오탐 4건(실사건 2건 포함) 해소, 차단 9건 전부 보존. `settings.json` 미변경 + 훅은 매 호출 디스크에서 읽힘 → **세션 재시작 불요**라 MINOR.
  - ⚠ 작성 중 밟은 함정: **awk 소스에 따옴표 리터럴을 쓰면 셸 single-quote가 조기 종료된다**(`syntax error near unexpected token`). `SQ=sprintf("%c",39); DQ=sprintf("%c",34)`로 유도했다. 첫 시도가 이걸로 죽어 회귀 14/14 전패 → 테스트 없이 커밋했으면 **훅이 모든 Bash를 차단**했을 것이다.
- **[체크포인트 "최신" 판정이 파일명 정렬이라 승인된 작업을 놓칠 뻔했다]** `session-check.sh:41`의 `sort -r`가 **파일명 = 시각**을 전제한다. `/context-save` 규약은 `YYYYMMDD-HHMMSS-`지만 수동 저장 파일엔 HHMMSS가 없고, `20260911-w…` 가 `20260911-09…`보다 앞선다(문자 > 숫자) → **00:10짜리 "할 일 없음"이 09:55 "트랙 3 승인됨"을 가렸다.**
  - 수정: frontmatter `timestamp:` 숫자만 추출 → **14자리 우측 0패딩** 키로 정렬, 없으면 mtime 폴백. 패딩이 없으면 `2026-09-11T00:10`(12자리)이 `…095544`(14자리)와 자리수가 어긋나 **또 오정렬**된다.
  - 실측: 실 체크포인트 10개 대조 — 종전 방식이 고르던 파일이 신규 키에서 **꼴찌**로 내려갔다. **재발 2회**(다른 워크트리에선 다른 브랜치 체크포인트를 안내).
  - ⚠ **미채택**: `branch:` 우선 필터. 회고 제안은 시각만 요구했고, 브랜치 우선은 **다른 브랜치의 더 최신 컨텍스트를 가릴 수 있다**. `/context-restore`(벤더)는 이미 브랜치 우선이라 훅만 시각축으로 맞췄다.
- **[pom 자가치유 sed가 CRLF를 죽이고, 그 손상본이 백업이 된다 — 3회 재발]** 자가치유 `sed -i`가 **치환 대상이 없어도** 실행돼, msys가 파일을 다시 쓰며 CR을 떼어냈다. 그 sed가 `cp pom pom.harnessbak`**보다 앞**이라 **trap이 되돌리는 백업이 이미 손상본**이다. 원복 수단이 `mv harnessbak`뿐이라 tester가 **금지된 `git checkout`으로 흘렀다 — 금지만 있고 대안이 없는 규칙**이었다.
  - 수정: ① `grep -q` 조건화(`tester-backend.md`·`tester-runtime.md` 둘 다) ② 원복 실패 절차에 **"`git diff --ignore-cr-at-eol --quiet` 가 참일 때만 `git show HEAD:<pom> > <pom>` 허용"** 명문화(내용 변경이 있으면 계속 금지 — 미커밋 변경 보호 유지) ③ `orchestrator.md:45` 요약 갱신.
  - 실측: `tocFramework/pom.xml` `i/crlf` 손상 / `tocAdminServer/pom.xml` `i/lf` 무해 = **파일별로 갈린다**. ⚠ **2회차는 orchestrator가 위임 프롬프트에 회피책을 명시 주입했는데도 재발했다** — 산문 주입이 진 자리다. ⚠ 회고가 제안한 `sed -b`는 **미검증이라 채택하지 않았다**.
- **[공유 `~/.m2`는 테스트 DB와 같은 층이다]** 워크트리 5개가 `crinity-toc-core:9.3.1`을 공유 — 10:46 install → 10:48 다른 세션 install로 교체 → 하위 모듈이 **이번 변경과 무관한 클래스**를 못 찾았다. **같은 날 두 번째 워크트리에서 재발**했으니 우연이 아니다.
  - `tester-backend.md ## 공유 아티팩트 저장소` 신설: install 직후 **식별 표지 확인 + 즉시 하위 빌드**(사이에 다른 작업 금지 — 창을 좁히는 게 유일한 회피), *"무관한 클래스 부재면 공유 jar 덮어쓰기를 먼저 의심"* 분류 규칙, **반환 계약 1줄**(역방향 파급이 더 조용하다 — 내가 install하면 남의 워크트리가 깨진다).
  - 착수 게이트(toplevel/branch)는 *잘못된 클론*만 막고 **정상 세션끼리의 동시 실행**은 못 막는다. wiki를 *공유 상태 저장소* 일반으로 확장했다(DB는 "순차만", `~/.m2`는 "창 좁히기"로 회피 폭이 다르다).
- **[도구가 낸 "0건"이 부재 근거로 쓰였다 — gitignore 축]** Glob·디렉터리 Grep은 ripgrep 기반이라 `.gitignore`를 존중한다. `@Test` Grep이 **0건**인데 디스크엔 **13/3/11건**이 있었고, 계획서도 Glob 0건을 "이미 삭제됨" 근거로 적었다(결론만 우연히 맞았다). orchestrator가 위임마다 경고를 **수동 주입**하던 자리 → `§0④` + `playbook-delegation ③` 규약화 + wiki.
  - **같은 뿌리의 짝 증상**: gitignore와 git 추적은 **배타가 아니다**. 옛 브랜치 커밋에 *추적돼 있던* 테스트 4개가 base엔 없어 **rebase 체크아웃이 디스크에서 지웠다** → 이력 재작성 뒤 변경검증 위임 전 `git diff --diff-filter=D` 1회를 orchestrator 규칙에 넣었다. 안 잡았으면 tester가 **없는 클래스를 지시받아 무음 부분실행**했을 것이다.
- **[측정식 자체가 참/거짓을 못 가르는 축]** `grep -c $'$'`가 Git Bash에서 **CR 유무와 무관하게 전체 줄 수**를 돌려준다. `lines=129 crlf=129`를 "129줄 모두 CRLF"로 읽고 사용자 보고 + tester 2명 위임에 그 전제를 실었다(tester 하나가 실측으로 반박, 정정 1회). 파일 손상은 0 — 비용은 **틀린 사실의 전파**다.
  - `§0④` 인코딩 항목이 종전엔 *"파이프가 **결과**를 오염"*만 다뤘다. **측정식이 아무것도 안 재는데 그럴듯한 숫자를 내는** 축을 추가했고, 일반 규칙을 하나 붙였다 — **"판정용 측정식은 답을 아는 반례 1개에 먼저 돌린다"**(LF 확정 파일 1개로 **30초**에 드러났다). 원인은 **미확인**으로 남겼다(msys grep 텍스트 모드 vs 도구 레이어 `$'…'` 해석).
- **[승인 게이트가 못 막는 오류 — 대상 목록의 소유권]** 주석 정리 22블록/10파일 중 공유 `config.xml` 4블록 가운데 **3블록이 merge-base 이전 제품 주석**(다른 기능, master 머지됨)이었다. 사용자 승인은 *"기준과 대상 목록"*에 대한 것이라 **목록에 섞인 소유권 오류는 게이트를 그대로 통과**한다. → `playbook-delegation`에 **`ⓞ 소유권 축`** 신설: 파일 전수 측정으로 대상을 뽑을 때 merge-base에 이미 있던 파일은 `git blame`으로 **WI 브랜치 커밋이 쓴 블록만** 남긴다.
- **[문서 위임은 보고를 믿기 쉽다 — claimed-but-not-applied의 비대칭]** "편집 위임 후 디스크 반영 검증" 규칙의 예시가 **코드 편집뿐**이었다. finalizer가 트레이스 ~90건을 *"모두 소스 대조 완료"*로 보고했으나 자가검증은 **스니펫만** 했고 설명 속 참조 **3단계 7건이 미갱신**이었다. → 문서·아티팩트 갱신도 동급 + **자가검증 범위가 보고 범위보다 좁은지 대조**.
  - ⚠ **독립 대조도 사람이 값을 옮겨 적는 순간 틀린다.** 같은 대조에서 orchestrator가 1건을 거꾸로 오판(**갱신본 값을 옛 값으로 착각**)해 정상 항목을 고쳤다가 재대조 스크립트가 잡아 되돌렸다. 판정은 **스크립트 출력(옛 값 → 기대값 → 실제값)으로만** 한다.
- **[rerere · 장문 커밋 메시지]** `finalizer.md ## 핵심 규칙` 2줄 — ① `rerere.enabled`가 켜져 있으면 rebase가 충돌을 *previous resolution*으로 채우고 **충돌 표시 0 · unmerged**로 멈춘다(**누가 언제 남긴 해결인지 모른다**) → 확정 해결안과 **바이트 대조 전 `git add` 금지** ② 다국어·장문 커밋 메시지는 heredoc 금지, **Write 파일화 + `git commit -F`**(실효 확인: `git log -1 %B` ↔ 원본 끝 개행 외 차이 0).
- **[reject 1건 — wi-trace `general-purpose`]** 대상이 **하네스 repo 밖**(`~/.claude/skills/wi-trace/`)이고, Step 2.5에서 **`general-purpose`가 현재 클코 내장 타입으로 실재**함을 확인해 원 증상의 전제가 사라졌다. 실제 거부는 타입 부재가 아니라 **역할 계약 충돌**(리뷰 전용 에이전트에 파일 쓰기)이었을 가능성이 높다.
  - 대신 `orchestrator.md:850`의 **사유를 교체**했다 — *"레지스트리에 없어 발사 실패"*(2026-08-13엔 맞았다)는 **지금은 사실이 아니다**. 규칙(read-only 브리핑엔 `code-reviewer`)은 유지하되 근거를 **역할 계약**으로 바꿨다. 틀린 근거를 단 규칙은 다음에 누가 그 근거를 확인하면 규칙째 버려진다.
- **[Step 2.5 — 2회 연속 서브에이전트 근거 오류]** 이번엔 *"훅은 `tool_name`만 보고 파일 경로·내용은 못 본다"*였는데, **우리 프로덕션 훅이 반증**한다(`block-developer-test-edit.sh:29`·`block-orchestrator-edit.sh:25`가 `tool_input.file_path`를 읽는다). 정정 후 **참조 Read `(n/2)` 강제는 실장 가능**으로 뒤집혔다.
  - 백로그 draft 2건(**미적용** — 훅 신설은 거버넌스): `2.5-A` 참조 Read 강제(`PreToolUse`+`agent_id`+`tool_input.file_path`), `2.5-B` 탐색 파일 수 상한(외부 state 카운트).
  - **없음**: 병렬 2소스 누락 감지 / rule 정량 사실 검증 / 의미론적 테스트 약화 / `-Dtest=` ↔ surefire 대조 / 스크린샷 육안 판정.
  - v5.13.0에 이어 두 번이다 — **Step 2.5 산출은 근거까지 따라가 검산해야 한다**는 게 이 Step의 상수 비용이다.
- **wiki 3건 신설 + 1건 확장**: [[gitignored-tests-invisible-to-glob]] · [[msys-sed-inplace-crlf-strip]] · [[gitbash-grep-cr-count-noop]] 신설, [[shared-test-db-worktree-noop]]를 *공유 상태 저장소(테스트 DB · `~/.m2`)* 로 일반화. CRLF 3종은 **같은 표면·다른 원인** 클러스터로 상호 링크(결과 오염 / 대상 오염 / 측정 불능). 정합성 `wiki-check.sh` OK(59페이지).

## 5.13.0 — 2026-09-09

> **클코 기능 대체 점검: 해당 없음 (클코 2.1.266)** — Step 2.5 최초 실행. 다음 드레인은 이 버전과 같으면 건너뛴다.

- **[Step 2.5 첫 실행에서 그 Step 자신의 결함이 드러났다]** v5.12.0에서 절차만 넣고 못 돌려봤는데, 실제로 돌리니 **추출이 거의 안 됐다.**
  - **자기매치**: 대상에 `skills/`가 들어 있어 이 SKILL.md의 **grep 라인 자체가 걸렸다** — 잡힌 3건 중 2건이 그것이라 실질 1건. → 대상을 `agents/ docs/`로 좁히고 `grep -v "harness-retro"` 추가.
  - **패턴이 좁았다**: `기계 강제가 없` 계열만 봐서 같은 뜻의 다른 표현을 전부 놓쳤다. `소프트 규칙`·`산문 규칙`·`재량 지시`·`탐지 수단이`·`그물이 어디에도`·`의심하지 않는다`·`유일한 확인선`·`흔적으로만`을 추가 → **9건** 수집(종전 실질 1건).
  - ⚠ **더 근본적인 것**: 탐색 파일 수 상한·스크린샷 육안 판정은 **강제 수단이 없는데 그 사실을 문면에 안 적어서** grep 밖에 있었다. 자동 추출은 **생산 쪽 습관이 전제**다 — 손으로 관리하는 위시리스트를 안 만드는 대신, 규칙을 **쓸 때** 그 사실을 남겨야 목록이 자란다. → Step 5에 짝 규칙 신설: **"강제 수단 없이 쓰는 규칙은 그 사실을 문면에 남긴다."** 이유 둘 — ① 읽는 에이전트가 그 규칙의 **신뢰 수준**을 안다(자기보고 축임을 알고 흔적을 남긴다) ② 안 적으면 클코가 나중에 덮을 수단을 내놔도 **영영 후보에 안 오른다.**
- **[첫 실행 결과 — 덮을 수 있는 새 수단 0건]** `claude-code-guide`에 9자리 질의(클코 2.1.266). 판정:
  - **불확실(문서 미명시)**: `SubagentStart` 입력 스키마, 도구별 **누적 호출 횟수**, `transcript_path` 구조 보장이 공식 문서에 없다 → 참조 Read 강제·탐색 횟수 상한·육안 판정 강제·위임 프롬프트 필수 슬롯 검사 **전부 실장 근거 부족**.
  - **병렬 tool_use 개수**: `Stop` 훅에서 `transcript_path`를 **파일로 직접 파싱**하면 *사후 검증만* 가능. payload에는 메시지 구조가 없다. 비효율 + 공식 보장 없음.
  - **rule 사실 오류**: N/A.
  - ⚠ **에이전트 답변 1건을 정정했다.** *"Bash 내부쓰기 차단은 PreToolUse(Bash)로 가능 — 근거: orchestrator.md"*라 했는데, 그 근거가 클코 문서가 아니라 **우리 파일 주석**(`hooks/block-developer-test-edit.sh:22`)이었다. 그 줄은 *"적극차단 **시** …"* = **제안**이고, 바로 위 20줄이 *"알려진 구멍(v1) — 현실 트리거 낮아 v1 제외"*라고 **우리가 의도적으로 미뤄둔** 것이다. 상태 변화 0. (서브에이전트 산출을 근거까지 따라가 검산해야 하는 이유 — `## 산출 수신 계약`.)
  - **역방향**: (a) 병렬 발사는 클코가 해결하지 않는다 → v5.10.0 *선언 금지* 유지가 맞다. (b) 서브에이전트 슬래시 스킬 제약 **여전히 유효** → `SKILL.md` Read 우회 계속.
- **[wiki [[orca-browser-e2e]] 플랫폼 조건 추가]** gstack v1.84.0.0으로 올리면서 **Aside** 브라우저가 1순위 드라이버가 됐다 — Aside도 **사용자 실제 로그인 세션**을 타므로 이 페이지의 "최대 이점"과 같은 자리다. **겹치나 확인했고, 안 겹친다**: `browse/SKILL.md`가 `uname -s`가 `Darwin`일 때만 Aside를 권고하고 **Windows·Linux는 헤드리스 폴백**(세션 공유 없음)이다. 이 머신은 `MINGW64_NT`, `aside` CLI 부재. → 플랫폼 표를 페이지에 넣고 `browser-probe.sh`의 **Orca 우선** 순서가 옳은 이유를 명시. 맥 사용자에겐 `/browse`를 권하면 된다.
  - 이건 Step 2.5 질문 형태(*"상류가 우리 우회책을 대체했나?"*)의 **gstack판 예행**이었다 — 답은 *"이 플랫폼에선 아니다"*.
- **부수 확인(조치 없음)**: gstack 업그레이드가 SKILL.md **52건을 "커스텀"으로 판정해 백업으로 밀어냈다**(`~/.gstack/backups/skills/20260909T175134`). 검사 결과 **사용자 편집이 아니다** — 한글 0건, `crinity`·`harness-setup`·하네스 고유어 참조 0건, 옛 파일이 일관되게 더 긴 구버전. setup이 자기 머신별 블록(gbrain·plan-tune)을 주입해 **설치본 ≠ pristine**이 되는 것을 "편집됨"으로 읽은 것. **복구 불필요.** `browse.exe`도 그대로 살아 있어(신규 `find-browse.exe`는 별개) 하네스 참조 4곳 무사.

## 5.12.0 — 2026-09-09

- **`harness-retro` Step 2.5 신설 — 클코 기능 대체 점검. MINOR(스킬 갱신).**
- **왜**: Claude Code 신규 기능·스킬을 상시 추적하는 게 버겁다는 사용자 통증. 진단은 **양이 아니라 필터 부재** — 나오는 것 대부분이 이 하네스와 무관한데 *무관하다는 판정*을 매번 사람이 한다. 상시 다이제스트(주 1회 스케줄)는 **소음이 규칙보다 커진다**고 보고 채택하지 않았다. **회고를 도는 김에 1회** 확인하는 쪽으로.
- **설계 3점**:
  - **입력을 자동 추출한다** — 손으로 관리하는 위시리스트는 반드시 stale해진다. `grep`으로 *"기계 강제가 없다 / 장치가 없다 / 프롬프트 룰뿐 / 자동 탐지"* 문면을 뽑는다(현재 **15건**: 탐색 한도, 참조 Read `(n/2)` 흔적, 육안 판정, rule 사실 검증 등). 규칙을 쓸 때마다 그 문면이 남으니 **목록이 저절로 갱신**된다.
  - **질의 내용이 이 Step의 전부다.** *"새 기능 뭐 나왔나"*는 소음 → **"이 N개 자리를 현재 클코 기능(훅 이벤트·settings·도구)으로 기계 강제할 수 있나? 덮을 수 있는 것만, 문서 근거와 함께. 없으면 '없음'."** 역방향도 함께 — **하네스가 우회책으로 만든 규칙이 이제 불필요해졌나**(예: 병렬 발사 누락을 *선언 금지*로 우회한 것).
  - **CHANGELOG 1줄이 마커다.** `클코 기능 대체 점검: 해당 없음 (클코 <버전>)` — 다음 드레인이 `claude --version`과 비교해 같으면 **건너뛴다**. 별도 state 파일·크론 없음. ⚠ 서브에이전트 사망·WebFetch 차단 시엔 **`미실행(사유)`**로 남긴다 — 조용히 생략하면 다음 드레인이 "이미 봤다"로 오인한다.
- **거버넌스**: 덮을 수단이 발견돼도 **자동 적용 금지** — 백로그 기재 후 Step 6 승인 게이트. 훅 신설은 MAJOR 후보다.
- 미채택: 주간 스케줄 다이제스트(B안) — inbox 큐에 push를 더 얹는 축이라 보류. 필요해지면 재론.
- ⚠ **첫 실행 미완**: 이번 커밋은 절차만 넣었다. 마커 줄이 아직 없으므로 **다음 드레인에서 최초 1회가 발동**한다(실측 없이 "해당 없음"을 적으면 날조다).

## 5.11.0 — 2026-09-09

> 회고 inbox 2파일(khnp WI 10) 드레인 — 후보 5건 중 **4건 적용 / 1건 보류**(계획 단계 흐름 색인 — 사용자 재고). 배치 1·2(v5.9.0·v5.10.0)와 같은 날.

- **[전제가 깨졌다 — "자동화 인프라 없음"이 기본값으로 굳어 있었다]** 렌더 룰의 *"인프라가 없으면(JSP·순수 jQuery 등) 사람 E2E 점검표로 이관"*이 그 스택에서 **면제처럼 작동**했다. 실제로 게이트 6종 전원 PASS 후 사용자가 화면을 열자 팝업 인라인 노출 → 수정 커밋 1라운드(2026-08-24, 이 사고가 그 룰에 이미 박혀 있었다).
  - **깨진 지점**: *"브라우저 자동화 = Playwright/vitest 같은 프론트 러너"*라는 암묵 전제. **Orca ADE 내장 브라우저는 CLI라 Bash만 있으면 몰 수 있고**, `tester-frontend`는 **이미 `tools:`에 `Bash`를 갖고 있다** → orchestrator가 직접 브라우저를 모는 게 아니라 **정식 위임**이다(역할 분리 불변).
  - 실측(2026-09-09): JSP + jQuery + 자체 컴포넌트 스택에서 E2E **4케이스 전부 자동 실행·판정** — 기간 상한 토스트 + 파라미터 3중 동기 / 월말 보정(`5/31 → 2/28`, JS `Date`가 3/3으로 넘기는 함정 회피, 경계 ±1초로 판별) / 플래그 토글 양방향(관리 UI 저장·원복까지) / 엑셀 전송 파라미터 가로채기 + xlsx **실물 파싱**.
  - **정정**: ① 렌더 룰에 **"`인프라 없음`을 선언하기 전에 프로브를 1회 돌린다"** — 판정 순서를 고정하고, 결과를 **tester 위임 컨텍스트에 주입**한다(tester가 스스로 탐지하지 않음 — `## codex 호출 가드`의 가용성 주입과 동형) ② `scripts/browser-probe.sh` **탐지 래퍼 신설** ③ `tester-frontend.md`에 Orca 절차 + 함정 3종.
  - ⚠ **머신 종속이 최대 리스크였다.** 하네스는 여러 머신·여러 사람이 공유하므로 `orca.exe` 절대경로를 규칙에 박으면 **그 경로가 없는 세션이 전부 깨진다.** → 탐지를 래퍼 한 곳에 모으고 **못 찾으면 조용히 사람 E2E 폴백**(차단 아님). 래퍼는 Orca(Win/mac/Linux 후보) → gstack `browse` 순으로 떨어진다.
- **[워크스루 산출 = 텍스트 브리핑 또는 `/wi-trace` 아티팩트 택1]** 둘이 **같은 목적으로 중복 저작** 중이었다(요청 흐름 재구성). 사용자가 실제로 wi-trace를 하나씩 짚어가며 인출한다 → 형식 택1 허용, **내용 요건은 동일**.
  - ⚠ **wi-trace가 담지 않는 3축은 별도 텍스트 필수 첨부** — 아티팩트로 갈음하면 조용히 빠진다: **3단계 계획 대비 이탈(양방향 — 특히 누락)** / **3.5 RED 기준선 대조** / **4단계 "여기만 읽어라"**. 누락 축은 [[gates-verify-present-code-only]]의 **유일한 그물**이다.
  - ⚠ **6단계 인출은 불변.** 아티팩트는 인출의 **입력**이지 대체물이 아니다 — 예쁜 문서가 있으면 *"문서 있으니 됐다"*로 새고 그러면 게이트가 죽는다.
  - **폴백**: wi-trace는 **WI 번호 전제**(브랜치 `<base>_WI_<N>`)라 WI 없는 트랙(풀사이클·개인실험)은 텍스트로. 아티팩트를 쓰면 기능 문서엔 **표 원문 + URL**을 함께 남긴다(문서가 SSOT, 아티팩트는 프레젠테이션).
  - **1차 판정 정정**: 최초에 *"v5.10.0 #16(워크스루 표 → wi-trace 입력)과 역방향"*이라 기각했으나 **충돌이 아니라 층이 다른 것**이었다 — 표=데이터, wi-trace=프레젠테이션. 사용자 실사용 근거로 반전.
- **[wi-trace가 데이터 객체를 통째로 놓친다]** 컨트롤러→서비스→DAO→매퍼 사슬만 이어서 그 사이를 흐르는 **Form·VO·BaseParam·DTO**가 스텝에서 빠졌다. 호출 관계가 없어 `endpoints`로도 안 잡히고 파일 단위 diff로는 "그냥 파일 하나"로 보인다. 실측: `MailMonitoringForm.java` **11줄 변경이 0줄 노출**, `AttachDownLoadLogListBaseParam.java` **67줄 전부 미노출** — 둘 다 그 WI의 기간 보정 로직이 사는 곳이었다.
  - **위 택1의 선행 조건**이라 같이 처리했다 — wi-trace가 **주 매체**가 되면 미노출 = 사용자가 **영영 못 봄**이고, 인출이 불완전한 지도 위에 선다. (편집 대상은 `~/.claude/skills/wi-trace/SKILL.md` — 글로벌 스킬이라 **이 bump 추적 대상 아님**.)
- **wiki 신설 [[orca-browser-e2e]]** — 함정 전부 실측: `snapshot`이 큰 페이지에서 **매번** `runtime_unavailable`(그 직후 `status`는 `ready` — 런타임은 멀쩡하다) → `eval` 대체 / `screenshot` 간헐 실패 → 재시도 + 결과가 파일 아닌 **JSON 안 base64** / **프로그램적 setter가 `onChange`를 안 쏘는 게 오히려 무기**("화면 값은 바뀌었는데 조회는 안 한 상태"를 정확히 만들어 내보내기 경로의 보정 로직만 단독으로 찌름) / 로그인 세션 공유 이점(MCP는 별도 프로필이라 계정·쿠키를 따로 받아야 한다).
  - ⚠ **범용/프로젝트 분리 적용**: 자체 컴포넌트 조작 규약 3종(합성 `MouseEvent` 무시, label 오버레이 클릭, `readonly` datepicker + 일요일·전후달 셀 클래스)은 **그 프로젝트 컴포넌트 라이브러리 지식**이라 wiki에 담지 않았다(소비자 오염 금지) → 해당 프로젝트 `rules/`. 일반화되는 교훈만 남겼다 — *"합성 이벤트가 안 먹으면 컴포넌트 API를 직접 호출"*, *"셀렉터를 '정상' 클래스로 좁히면 예외 케이스에서 실패한다"*.
- **보류 — [계획 단계 요청→응답 흐름 색인]** (백로그 `deferred`, 사용자 재고 중)
  - **문제**: wi-trace를 WI 마지막에 돌리면 *"다 결정되고 개발된 다음"*이다. 흐름이 틀렸다는 걸 그때 알면 바꾸는 비용이 최대다.
  - **진단**: wi-trace는 `git diff` 전제라 **앞당기는 게 구조적으로 불가능**하다 — 앞당길 대상이 wi-trace가 아니다. 그리고 계획 단계에 **흐름 데이터는 이미 4중으로 있다**(co-plan Step 1 / co-plan Step 4~5 / planner `## 흐름 N:` — `check-planner-output.sh`가 기계 검증 / planner 유저 시나리오). **최초 제안(흐름 스켈레톤 섹션 신설)은 중복이라 철회.**
  - **진짜 갭 = 데이터가 아니라 프레젠테이션.** 흐름 정보가 3곳에 흩어지고, 요청→응답이 한 화면에 이어져 보이지 않고, 계획서가 720줄일 수 있다. **앞당길 것은 정보 생산이 아니라 렌더링이다.**
  - 샘플 아티팩트를 만들어 형식을 검토 중(계획/구현 후 토글로 **같은 표가 두 시점을 산다**, `데이터` 배지로 Form/BaseParam 사슬 편입, `이탈` 배지는 구현 후 뷰에만). 미해결: planner 자동 생성 규약(계획서에서 계층·순서·구분을 뽑는 방법) · 토글 vs 2열 병치 · 흐름 접기 · 발행 승인 프롬프트 비용. 재개 지점은 백로그에 상세 기록.

## 5.10.0 — 2026-09-09

> 회고 inbox 6파일 드레인 배치 **2/2** — 규칙 축 전량(코드 버그 3건은 v5.9.0). 후보 27건 중 **24건 적용 / 1건 reject / 2건 별건 보류**. + 사용자 요청 2건(워크스루 계층 인벤토리 · wi-trace 연동).

**rules 축 (사용자 지목)**
- **[rule 문서의 사실 오류가 SSOT로 전파된다]** rule의 *"테스트가 2개뿐"*이 실측 `@Test` **0건**이었고, 그 서술이 계획서 회귀범위표 → `-Dtest=` 목록으로 전파돼 surefire 무음 스킵을 낳았다. **"테스트 2개"와 "0개"는 회귀 전략이 갈리는 차이**다. 핵심은 개별 오타가 아니다 — **rule은 SSOT로 취급되는데 게이트는 rule을 기준으로 삼지 rule을 의심하지 않는다.** v5.6.0의 *"승인 문서가 단언하는 외부 사실은 실측"*이 **계획서 축**만 덮고 rule 축이 비어 있었다. → `skills/rule-maker/SKILL.md` 정량 서술 규칙(파일 개수 아닌 `@Test` 개수 + 근거 병기) + `orchestrator.md`에 **rule판** 실측 규칙.
- **[주석 관례를 rules → agent md로 승격]** `.claude/rules/`는 **이중 gitignore**(제품 repo `.claude/` + 하네스 `rules/`)라 어느 git 이력에도 안 남고 **그 PC 그 프로젝트에만** 적용된다. 프로젝트 무관 원칙 6개를 `developer-*.md ## 주석 작성 관례`로 승격 — 호출부는 "값의 정체 → 화면에서 뭘로 보이나" 순 / 용어 던지기 금지 / **계획서 조항번호 금지**(`흐름 0`·`금지 27`) / javadoc 중복 금지 / UI 문구 인용은 **verbatim 테스트로 잠겼을 때만** / 인용값 변경 시 옛 값 전수 grep. ⚠ 부수: **orchestrator가 지시한 주석 자체가 이 원칙을 어겨 반려됐다** → 위임 프롬프트에도 같은 기준. wiki [[comment-style-purpose-first]] 신설(왜=wiki, 강제=agent md).

**게이트 판정 축**
- **[tautological 테스트를 7.6이 구조적으로 못 잡는다]** 7.6 판정축이 *"FAIL이 올바른 이유인가"* 하나뿐인데 **tautological은 FAIL하지 않는다 — PASS한다.** 그래서 판정표에 아예 안 걸린다. 게다가 PASS 자체는 결함이 아니다(정당한 negative 대조군·회귀가드도 GREEN 전 PASS) — **셋을 구분하는 축이 없었다.** → **currently-PASS 3분류** + **(c) 1건이면 불통과**(차단 권한). 판정법 = *단언 결과를 목 입력이 전부 결정하는가*. 7.5 작성자의 **PASS 예상 케이스 지목을 선택 → 필수**로. 근거: 문제 케이스가 7a·7b·7.5·7.6을 **전부 통과**했고, 7.6은 **이미 감지했는데 차단 권한이 없어 비차단 관찰로 흘려보냈다**.
- **[RED는 "첫 실패 지점"까지만 검증한다]** 7.6을 4회 PASS했는데 미검증 코드가 최소 3곳, 그중 하나에 P3 **필수잠금**이 의존했다 — 필터가 틀렸으면 **잠긴 척만** 하는 상태. 정규식은 특히 조용하다(`PatternSyntaxException`은 도달해야 난다 → **로그가 깨끗하고 그 깨끗함이 유효성의 증거처럼 읽힌다**). → tester-backend **도달성 보고** + 미도달 구성물이 필수잠금·보안에 걸리면 **독립 probe**(scratchpad, 테스트 파일 무수정).
- **[`-Dtest=`는 존재하기만 하면 통과한다]** `@Test` 0건 + `main()` 스크래치를 surefire가 **에러·경고·리포트 없이** 건너뛴다(지시 7 → 실행 5). 오타는 죽지만 **"존재하는데 테스트가 아닌" 경우는 성공적으로 아무것도 안 한다.** → planner가 `@Test` 개수 grep·병기 + tester가 **지시 ↔ surefire 리포트 대조표**. 종전엔 orchestrator 재량 지시로만 잡혔다 — **탐지 수단이 단 하나였다.**
- **[테스트 인프라 코드가 단일 실패점]** 공유 헬퍼 **하나**가 케이스 **24건을 한꺼번에** 죽였다(`getParentFile()`이 FS 접근 없이 문자열만 파싱 → 자기 자신 반환 / `static` 초기화 실패가 클래스 전체 전이). **CWD 의존이라 조용하다** — repo 루트에서 돌렸으면 우연히 통과했다. 같은 스위트에 **올바른 구현이 이미 있었다**. → tester-design **R24**.
- **[작성자 자기보고 ≠ 실물]** 3건(반증이 **산문으로만** 충족 / 기대값이 구현 전 값과 **우연히 동일** / **존재하지 않는 테스트** 인용) 전부 규칙 아닌 임기응변으로 잡혔다. 7.7 루브릭은 *"작성된 것의 품질"*을 보는데 이건 **"작성됐다는 주장의 참/거짓"**이라 축이 다르다. → tester-quality **직접 세어 대조**(불일치 critical) + *"X가 커버한다"*는 **grep 실재 확인**.

**orchestrator 규율**
- **[작업 루트가 안 박혀 엉뚱한 저장소를 검증했다]** 패널엔 `repoRoot` 주입이 **필수 명문화**인데 **tester/developer 위임엔 같은 규칙이 없었다** — 패널만 보호되고 실행 에이전트는 무방비. 잘못된 클론의 보고는 **그 저장소 기준으로는 전부 참**이라 자기모순 없는 **그럴듯한 오보**다. 직전 grep 기록이 없었으면 **멀쩡한 RED 47건을 재작성**시켰을 것이다. 부수로 `~/.m2` 공유 jar가 타 브랜치 소스로 오염됐다. → 작업 루트 절대경로 + **착수 게이트**(`rev-parse`/`branch`/`status` 기대값). ⚠ `.claude`가 심링크면 **경로만으로 루트를 추정할 수 없다.**
- **[병렬 발사 규칙을 뒤집었다 — 대조 → 선언 금지]** 종전 *"세어서 대조하라"*가 **6/6 전패** + 타 프로젝트 재발. 실패가 구조적이다 — 규칙은 **사후** 검사인데 실패는 **생성 시점**에 일어난다. → **"산문으로 예고하지 마라. 먼저 보내고 나중에 서술한다."** 선언이 없으면 갭도 없다. (훅 기계화는 **거부** — 차단 훅 신설 MAJOR + 오탐원 추가. 마침 v5.9.0에서 훅 오탐을 걷어내는 중이었다.)
- **[채택한 recommendation을 검산 없이 지시문으로 복사]** 타당성 게이트 ①~⑤는 **기각 판정용**이고 *채택* 경로엔 검산이 없어 **무비판 전달이 기본값**이었다. 한 트랙에서 2회, 둘 다 다음 라운드 finding으로 회수(= 없었으면 shipped). 하나는 **ADR·코드 주석에 박힐 예정**이었고, 다른 하나는 지시대로 쓰면 **RED가 성립하지 않는다**. → **⑥ 과잉일반화 / 그대로 구현하면 성립하나** 검산.
- **[폴백 보상이 blocking 0이면 공허하다]** 폴백 문면이 전부 *"blocking 인용라인 대조"*를 전제하는데 단일 소스가 blocking 0을 내면 **보상 대상이 0개**다 — 태그만 붙고 추가 검증은 **0**. [[gates-verify-present-code-only]] 클래스(워크스루 3·3.5에서 이미 메운 사각인데 **리뷰 폴백만 안 메워져 있었다**). → **PASS 근거 실측**으로 전환(안전 임계 2~3개 직접 Read).
- **[승인화면 대가 3분류]** 계획서 문안이 **미룬 대가만 덮고 수용 대가는 하나도 안 덮었다** — 패널이 *"폭발반경 역전"* 지적(미룬 것보다 수용한 것이 강한데 고지는 더 약함). → ①수용 ②미룬 ③미수리 **3칸 필수**(빈 칸은 "없음" 명시) + **계획서 문안을 그대로 쓰지 않고 findings 전량으로 재조립**.
- **[모델 실측 명령이 옆 프로젝트를 물어왔다]** `projects/*/*/`가 mtime으로만 정렬 — 멀티 워크트리에선 **오염이 기본값**이다. 1차 측정 전건 `sonnet`(실제는 `fable-5`) → 믿었으면 재스폰 1라운드. **반대 방향이 더 나쁘다**(옆이 fable로 돌면 **진짜 강등을 정상으로 읽고 통과**). → `PROJ` 고정 + `echo "dir: $D"` 확인 의무(오염을 잡은 게 정확히 이 습관인데 규칙엔 없었다). 함께: **`429` = 폴백 정상이 아니다** — 크레딧이 마르면 **폴백 없이 에이전트가 죽는다**(산출 0). 종전 문면이 2026-07-28의 *폴백 성공* 사례만 담고 있었다 → **산출 있으면** 모델 판정 / **산출 0이면** 사망 행으로 분기.
- **[codex 능력 경계]** ① 대형 입력(1000줄+/5파일+)엔 **탐색 예산**을 프롬프트 선두에(도구 상한·부분 읽기 범위·*"무출력 종료가 최악"*) — 없으면 **에러 없이 exit 1**로 무음 사망(로그 512KB까지 탐색만, 산출 0). 4요소 주입 후 완주 실측. ② **테스트·빌드 실행을 요구하지 마라** — `-s read-only`에서 Vite/vitest가 임시 config 쓰기 **EPERM**.
- **[인코딩 검증은 파이프를 경유하지 않는다]** `sed`/`echo`/`perl`이 백슬래시를 재해석해 **정상을 결함으로** 보이게 한다 — 정상 `.properties`를 "이스케이프 깨짐"으로 판정하고 결함 보고까지 썼다(**측정 도구가 결과를 오염**). raw 출력 또는 대조군 비교.
- **[오귀속 차단]** 실측으로 "오늘 X / 변경 후 Y"를 쥐고도 *"메커니즘은 기존이니 신규 아님"*으로 판정 → 패널이 **그 출력 자체를 근거로** 반박. **X ≠ Y면 신규 대가다**(메커니즘 기존은 완화 근거일 뿐). 같은 세션에 planner의 동형 오류를 critical로 판정하면서 **자기 판정엔 같은 잣대를 안 댔다.**
- **[절대 단언 규율 — planner ×3]** `유일`·`불가`·`완전히`·`항상`·`무변경`은 근거 병기 또는 범위 한정, 둘 다 못 하면 **"확인 못 함"**. 계획서는 설득력을 위해 절대어를 쓰고 게이트는 **정확히 그 절대어를 반증 대상으로 삼는다** — 구조적으로 라운드를 생산한다. 실코드 **1줄**에 3라운드가 든 트랙에서 라운드 1~2 findings 대부분이 **문서 주장의 과장**이었고, 규칙 주입 후 **codex major 3 → 0**.
- **[전체회귀 부채]** 방금 전체 스위트를 돈 직후 *"전체회귀 강력 권장"*이 떠서 **방금 돈 걸 다시 돌라고** 권하는 모양. → **문구만 병기**(로직 무변경). tester-runtime 리셋 독점은 **정상 동작**이다(전체회귀 + 기동/통합은 범위가 실제로 다르다 — 전체 리셋 허용은 기동 공백을 부채 0으로 위장). ⚠ 이상하면 v5.9.0 `core.quotepath` 버그부터 의심.
- **[codex 가용성 라운드마다 재주입]** 1회차 주입 → 2회차 누락 실측. tester 위임 템플릿 **필수 슬롯**.

**사용자 요청**
- **[워크스루 계층별 변경 인벤토리]** 종전 1단계는 흐름 **서사 한 줄**이라 어디가 추가/수정인지 **델타 표기가 없었고**, 3단계 이탈 대조는 **파일 단위**(`git diff --name-only`)라 메서드가 안 보였다. → 요청→Controller→Service→DAO→DB→응답 **호출 순서** 표(클래스#메서드·`file:line`·신규/수정·**왜**). 미변경 경유도 `경유(무변경)`으로 1행(빠지면 지도가 끊긴다). 표 ≤12행, `반환 ≤30줄` 계산에서 제외.
- **[wi-trace 연동 — 마지막에 할 일은 이해가 아니라 조립]** wi-trace는 **WI 브랜치 전체**를 git diff에서 처음부터 쪼개는데, 그 트랙들은 **각각 이미 워크스루를 통과**했다 = 같은 노동 2회, 비용은 트랙 총합. wi-trace 스킬 본문도 인정한다 — *"`endpoints`로 요청 분할이 끝나지 않는다. 호출 관계가 없어서 단계 순서를 못 준다."* **그 빠진 정보를 워크스루가 갖고 있는데 안 넘겨주고 있었다.** → 헤더 `— WI N / 트랙 M: 제목` 고정(정렬 키), wi-trace가 누적본을 **뼈대**로 쓰고 `git diff`는 **누락 대조용**(없으면 현행 폴백). **`왜` 칸을 트랙 시점에 채우는 게 핵심** — 설계 결정은 승인 시점에만 살아있고 구현이 끝나면 휘발한다.
  - **사용자가 흐름을 잃지 않게**: 표 앞에 누적 2줄(`이 WI 누적 흐름 N단계` / `이번 트랙이 건드리는 것`). ⚠ 둘째 줄이 핵심 — 이번 트랙이 **앞 트랙이 만든 것을 고치면** 명시한다. 없으면 매번 새로 보는 느낌이 들고 거기서 이해가 끊긴다.

**reject / 보류**
- ❌ **forge 위키 파일명 규약** — 프로젝트 운영지식(소비자 오염 금지). 해당 repo `agent-memory`·ADR에 이미 기록됨.
- ⏸ **`/spec` 라우팅 · 목업 게이트 확대** — *기능 자체가 요구와 다른* (b)축. 별건.
- 📌 **성공사례(유지·강화)**: 스킬 전제조건 폴백 계약이 값을 했다 — `/code-review` 폴백의 *"untracked 신규 20파일 전수 Read"*가 결정적이었다(`git diff`만 봤으면 이번 변경의 대부분을 통째로 놓쳤다).

**하네스 repo 밖**: `~/.claude/skills/wi-trace/SKILL.md` 진입 규약(누적본 우선)은 글로벌 스킬이라 이 bump 추적 대상이 아니다 — 별도 편집.

## 5.9.0 — 2026-09-09

> 회고 inbox 6파일 드레인 배치 **1/2** — **무음 실패 3건**만 먼저 뗀다. 셋 다 지금 이 순간에도 거짓 안심을 생산 중이라 규칙 항목들과 같은 커밋에 묶지 않았다. (규칙 축은 v5.10.0.)

- **[N1] `red-baseline.sh`가 gitignored 테스트를 조용히 스킵하고 `✅ 이탈 없음`을 출력했다 — 워크스루 3.5가 통째로 무력화.**
  - `:74`의 `git ls-files -co --exclude-standard`가 `.gitignore`를 적용한다. 대상 프로젝트는 루트 `.gitignore`에 `*/src/test/`가 있어(테스트 추적 해제) **테스트 전량이 ignored** → 수집 0. 그런데 보고된 61파일은 **전부 타 모듈의 tracked 테스트**였고 **이번 기능의 RED 6파일은 한 건도 없었다**(2026-09-07 실측).
  - **핵심은 스크립트가 "대상 0"과 "이탈 0"을 구분하지 않는 것**이다. 출력이 동일하게 `✅`라 orchestrator는 정상 통과로 읽는다. `wiki/gates-verify-present-code-only.md`의 *"부재를 통과로 읽지 않는다"* 원칙 위반이 **스크립트 층**에 있었다.
  - 발견 경위도 위태롭다 — tester-design이 3케이스를 append한 **직후에도** "이탈 없음"이 나와서 이상을 감지했다. **테스트가 방금 바뀐 걸 아는 상태가 아니었으면 못 잡는다.**
  - 정정: ① 수집을 `tracked+untracked` ∪ `ignored`로 바꾸고 벤더 경로(`node_modules`/`target`/`build`/…)만 `VENDOR_RE`로 배제 ② 수집 0이면 `⚠ 미검증` + **exit 3**(종전 중립 문구 "생략" → 경고) ③ 스냅샷 성공 출력에 **모듈 분포** 1줄(총계만 보면 "우리 모듈 0"이 안 보인다).
- **[N2] `regression-debt.sh:176` — git `core.quotepath`로 제외 규칙이 무음 무력화.**
  - 기본값 `quotepath=true`가 비-ASCII 경로를 `"docs/guide/\354\206\214..."`로 큰따옴표+8진 이스케이프해 출력 → `derive_modules():91`의 `docs/*` 글롭이 **선두 `"` 때문에 매칭 실패** → `*/*`로 흘러 모듈명이 `"docs`가 된다.
  - "카운트가 1 틀렸다" 수준이 아니다. **제외 규칙(`.claude/*|docs/*`) 자체가 조용히 무력화**되고, `src/` 아래 파일도 `"src`로 산출돼 **같은 모듈이 두 이름으로 집계**된다. 하네스는 여러 프로젝트가 공유하므로 영향이 한 프로젝트에 갇히지 않는다.
  - 정정: `git -c core.quotepath=false show …` (1줄). 파일 경로를 파싱하는 git 호출은 이 스크립트에 `:176` 1곳뿐임을 grep 확인.
- **[N3] 백스톱 훅 — 오발 2종 + 무한 차단 가능.**
  - **오발**: `:46`이 체크포인트 본문 어디서든 `[LOOP 2/3]` 리터럴을 grep했다. 그 문자열은 **상태 마커인 동시에 산문 표기**라 구분이 안 된다 — ① 예고문(*"critical 있으면 반환 [LOOP 2/3]"*, 실제 루프는 1/3) ② **종결 기록**(*"→ **통과** [LOOP 2/3]"*, 루프는 이미 끝남)까지 "진행 중"으로 읽고 turn 종료를 막았다. 게다가 `## 작업 컨텍스트 보존`이 이 리터럴 기록을 **의무화**한다 = **규칙을 지키는 행위가 트리거를 생성**하는 구조였다.
  - **무한 차단**: `:73`이 `printf … > "$STAMP" 2>/dev/null || true`로 **쓰기 실패를 삼킨다.** 스탬프가 안 써지면 다음 Stop에서 지문이 또 baseline과 달라 **매번 재차단**한다. 루프 방지 주석이 전제하는 갱신이 실제로 실패할 수 있고(실측: 스탬프 mtime이 세션 시작에 멈춤), 훅은 그걸 **알려주지도 않는다**.
  - 정정: ① 앵커를 **전용 필드** `^\s*loop_status:\s*[23]/3`로 교체 ② 쓰기 실패 시 **차단하지 않고**(fail-open) stderr 경고 후 exit 0.
  - ⚠ **짝 편집 필수**: `orchestrator.md ## 작업 컨텍스트 보존`이 `loop_status: n/3` 필드를 쓰도록 바꾸고 **산문 리터럴을 금지**했다(설명이 필요하면 `LOOP 2-of-3`). 훅만 고치면 신호가 영영 안 잡히고, 문서만 고치면 오발이 남는다 — **분리 커밋 금지 대상**이라 같이 넣었다.
- bump 판정: N3이 차단 훅 변경이지만 **신설이 아니라 차단을 좁히는 버그 수정**(오발 제거 + fail-open)이라 MINOR. 게이트 구조·판정 로직 불변. (사용자 확인 2026-09-09.)
- **미적용(배치 2/2로)**: 규칙 축 전량 — rules 사실오류 전파, 주석 관례 승격, 7.6 tautological 판정축, 워크스루 계층 인벤토리 등.

## 5.8.0 — 2026-08-30
- **[developer가 백지에서 쓴다] planner→developer 참조 앵커 체인 신설 — MINOR. 재시작 권장.**
- **갭의 정체** (사용자 제기: *"요구랑 결과물이 차이가 많이 난다 / 코드 스타일도 튄다 / 수정 요청이 너무 많다"*):
  - planner 출력 `### 수정 대상`은 **고칠 파일**만 나열하고 *"이걸 닮게 써라"* 파일을 지목하는 칸이 **없었다.**
  - `agents/developer/` 전체 grep — "기존 유사 파일을 읽고 그 패턴을 따르라"는 지시 **0건**(유일 히트 `developer-backend.md:63`은 null 처리 한정 한 줄).
  - developer가 읽는 건 `.claude/rules/package/<모듈>/*.md` = **서술된 규칙**뿐. 텍스트 규칙은 "taglib 헤더" 같은 **명시 규칙**은 옮기지만 **명명 관습·구조 리듬·함수 분해 크기 같은 암묵 스타일은 못 옮긴다.** → **룰 PASS와 스타일 튐이 동시에 성립**하는 구조였다.
  - 결과: developer는 매 파일 **백지에서** 쓴다. 신입에게 컨벤션 문서만 주고 기존 코드는 안 보여준 것과 같다.
- **정정 — 앵커 체인 5지점**(하나라도 빠지면 끊긴다):
  - `planner-{frontend,backend,high-complexity}.md` 출력형식에 **`### 참조 구현` 섹션 신설**(수정 대상 직후 — 고칠 파일과 닮을 파일이 붙어야 한다). 선정 기준 = ① 같은 모듈 ② 같은 **종류**(기능 유사 아님, **형태** 유사) ③ **최근 수정분**(`git log -1`로 legacy 회피). **최대 2행.**
  - 같은 3파일 **반환 계약**에 `### 참조 구현` 표를 **전문 예외**로 추가. 종전 "섹션별 1~2줄 요약"이면 경로가 요약에 녹아 developer까지 **도달하지 않는다.**
  - `developer-{frontend,backend}.md`에 **`## 참조 구현 (첫 Write 전 필수)`** 신설(코딩규칙 Read 직후·핵심규칙 앞).
  - `orchestrator.md` 위임 규칙에 **"`### 참조 구현` 표를 컨텍스트에 그대로 실어 보낸다"** 추가.
- **⚠ 탐색 한도와의 관계 (사용자 지적으로 교정)**: 초안은 참조 Read를 *"탐색 5+5 한도와 별개"*라고만 써 **상한을 안 박았다** — "별개 = 무제한"으로 읽혀 폭주 방지선에 구멍이 난다. → **참조 Read 상한 = 2파일**로 명시.
  - 별도 예산이 정당한 이유: **탐색과 지목 Read는 비용 축이 다르다.** 폭주는 *"어디 있지?"* 헤맴(Glob·Grep 반복)에서 나고, 참조는 planner가 경로를 박아준 **직행 Read라 헤맴이 0**이다. 한도는 "얼마나"만 정하고 **"무엇을"은 안 정한다** — 종전엔 5슬롯이 수정 대상 확인에 다 쓰여 **참조용 슬롯이 사실상 0**이었다. 원인은 한도가 아니라 **타겟팅 부재**였다.
- **나쁜 참조 방어**: 참조 파일의 버그·안티패턴·deprecated API는 **복제 금지**, 충돌 시 **우선순위 = `rules/` > 참조 파일**(참조는 암묵 스타일 공급원이지 규칙 SSOT가 아니다). 선례가 없으면 억지 지목 대신 **`참조 없음: <종류> — 사유`를 정식 출력으로 인정**(억지 지목이 더 해롭다).
- **⚠ 기계 강제 없음 — 알려진 한계**: 탐색 한도도 이 참조 룰도 **프롬프트 룰뿐**이다(`hooks/` 10종에 탐색 제한 훅 0건, 계측도 없음). 실효 확인은 developer 반환의 **`참조 Read: <경로> (n/2)` 흔적**뿐 — 그래서 그 1줄을 반환 계약에 넣었다. 훅 승격은 별건.
- **미적용**: (b)축 = *기능 자체가 요구와 다른* 갭(gstack `/spec` 미라우팅, 목업 게이트가 **신규화면만** 발동 `:547`). 별건으로 남김.
...
- **철회 사유 (능동적 해악)**: `jdtls` 서버가 미설치라 호출 시 **120초 타임아웃**을 태우고 아무것도 못 얻는다. 규칙을 안 써도 도구 목록에 보이면 Java 작업 중 시도할 유인이 있다 — 라운드 낭비를 줄이자고 만든 하네스에 **2분짜리 함정**을 심는 꼴이다. 게다가 v5.5.0에서 *"선언은 보이는데 능력은 없다"*를 wiki 함정으로 박아놓고 그 상태를 남기는 건 앞뒤가 안 맞는다.
- **유지**: [[agent-tools-silent-drop]] wiki. 이번 라운드의 산출은 LSP가 아니라 **무음 드롭 + 메인/서브 비대칭** 지식이고, 도구 권한을 다룰 때마다 계속 쓰인다.
- **재론 조건 (하나로 고정)**: **서브에이전트가 플러그인 도구를 받게 되면** 그때 다시 본다. 그 축이 안 열리는 한 계산은 안 바뀐다.
...
- **⭐ [B] 계측을 산문 규칙에서 워크플로로 옮겼다 — "메커니즘은 있고 호출이 없던" 무음 실패 해소.**
  - v4.7.0이 *"측정 없이 게이트 안 건드린다"*며 `panel-metrics.sh`를 신설하고 기록을 `orchestrator.md` 산문 절차(패널 반환 후 `0-a`)에 뒀다. **7일간 전 프로젝트 8곳 전수 0건**이었다 — 그 사이 패널은 4라운드 돌았다. 스크립트는 정상 동작(직접 호출로 실증)했고 **부르는 쪽이 빠졌다.**
  - 아이러니: 계측이 0이라 v4.7.0이 판단을 미뤄둔 그 안건(재게이트 페르소나 전원 스폰이 정당한가, 근거 n=1)을 **여전히 못 한다** — 커밋 하나가 7일간 순수 비용이었다.
  - **기계강제 경로 실측**: ① 스크립트 직접 append ❌ — `design-panel.js`는 JS 샌드박스(Bash/exec/require 0건, 파일시스템·Node API 없음) ② 훅 ❌ — `settings.json` 매처가 `Bash`/`Edit|Write|MultiEdit`/`SubagentStop`(에이전트명)/`Stop`이라 **Workflow 도구에 걸리는 매처가 없다**. → **③ 워크플로가 반환 직전 agent 1개로 대행**(비용 ≈2~3k 토큰 = 패널 1회의 1% 미만, `model: 'haiku'`).
  - 비차단 보장: `try/catch`로 감싸고 스크립트 자체가 전 실패에서 exit 0 + stdout 미사용. **게이트 판정에 영향 없음.**
  - `round`는 워크플로가 모른다(재게이트 = 재호출이라 리셋) → args로 오면 쓰고 없으면 `isReGate ? 2 : 1`. 핵심 파생지표 `ripplePersonas`는 `priorPersonas ↔ perPersona` 대조라 round 없이 성립하고, `priorPersonas`는 `priorCriticals`에서 직접 유도해 **새 args가 필요 없다**(계약 점검 통과).
- **[A] orchestrator 감축 — 삭제가 아니라 이동. 규칙 손실 0.**
  - **A1 — co-plan OOP 컨텍스트 블록(19줄) → `skills/co-plan/SKILL.md` 본문.** v5.3.0에서 그 스킬이 **repo SSOT**임이 확인돼(`sync-skills.sh` 동기화 대상은 grill-with-docs뿐) 주입할 이유가 사라졌다. orchestrator가 주입하는 건 이제 **0단계에서 정해지는 rule 경로 한 줄뿐**이다.
  - **A2 — `### 세션 끊김 후 복구`(10줄) → `playbook-delegation.md ## 백그라운드 패널 세션끊김 복구`.** 트리거-조건부 절차고, 그 playbook이 이미 이 절을 "별도 절차"로 크로스레퍼런스하고 있어 **짝을 한곳에 모은 것**. orchestrator엔 트리거 1줄 + "`resumeFromRunId`는 same-session 캐시" 한 줄만 남긴다.
  - **A3(`gstack 스킬 라우팅` 46줄)은 미측정 — 후보 등록만.** 상시 필요한 라우팅 표라 근거 없이 손대지 않는다.
  - **[중복 통합]** v5.2.0에 내가 넣은 playbook `도구셋 확인` 블록이 기존 `### ③ 대상 에이전트 도구셋 확인`과 겹쳤다 — 통합하고 Read 시점 목록을 4개(계약저술 / 실행요구 / 사망 / 패널끊김)로 정리.
- **📉 실측 — 순감 전환**: `orchestrator.md` **968→945줄 / 13,128→12,960단어(−168)**. v5.1.0 기준(960줄/12,517단어)에 대해 **줄수는 −15로 아래**, 단어는 +443이 남았다(v5.2.0 드레인 15건분). **38일 단조증가 후 첫 순감 커밋.**
- 라우팅 판정: `harness-drift-check.sh`가 co-plan 절 제목 변경을 drift 의심으로 올렸으나 **단계 신설·삭제가 아니고** `routing-map.md:41`의 `/co-plan(OOP5)`가 여전히 정확해 다이어그램 갱신 불요(비차단 무시).
- ⚠ WARN: `sync-skills.sh` `⛔ 대조된 스킬 0개` — **6회 연속**. 다음 배치에서 SOURCES 맵 정리할 것.

## 5.3.0 — 2026-08-14
- **co-plan Step 4~5가 기존 파일 수정을 신규와 동일 취급하던 것 분리 — MINOR. 재시작 권장.** (사용자 요청)
- **문제**: 종전 기준은 클래스·메서드 모두 *"목록 + **역할 한 줄**"*이었다. 신규 클래스엔 충분하지만 **기존 파일 수정엔 판정 근거가 안 된다** — 유저가 ① 그 클래스가 지금 무엇을 하는지 ② 이 수정이 기존 동작을 어떻게 바꾸는지 ③ 어디에 무슨 코드가 들어가는지를 **알 수 없어 수정 범위를 스스로 판정할 수 없다**. Step 3~5는 자동 진행(합의 왕복 없음)이라 유저에게 남는 건 그 출력뿐인데, 그 출력이 신규 기준으로 쓰여 있었다.
- **정정 — 신규 / 기존수정 2분기**:
  - **클래스(Step 4) 기존수정**: 파일 경로 + **현재 역할**(지금 무엇을 책임지고 누구와 협력하나) + **`현재: … → 변경 후: …` 대조**(유저가 보는 차이로) + **변경 성격**(책임 추가 / 기존 분기 변경 / 계약 변경 — 계약이면 소비자 영향 1줄).
  - **메서드(Step 5) 기존수정**: `파일:라인 · 시그니처` + **현재 하는 일** + **삽입·교체 지점**(어느 분기 안 + **앵커 코드 1줄 인용** — 라인 번호는 편집으로 밀리므로 번호 단독 금지) + **들어갈 소스 스케치**(시그니처가 아니라 실제 코드 몇 줄) + **호출측에서 달라지는 것**.
  - 형식 예시를 SKILL.md에 인라인(신규는 종전 한 줄 유지 — 해상도 요구가 다르다).
- ⚠ **현재 역할·현재 동작은 실제 소스 Read 근거로 쓴다.** 못 읽었으면 추측 금지 → `⚠ 미확인 — 추정` 라벨. 그 라벨은 planner 계획서 baseline까지 **그대로 승계**한다(조용히 확정으로 바꾸지 않는다 — 설계패널·승인 게이트가 그 축을 의심할 수 있게). §0④ 데이터 전제 검증의 co-plan판.
- **출력 폭주 가드**: 기존수정 항목 6개 초과 시 영향 큰 5개만 상세 + 나머지는 파일·메서드명만.
- **[stale 정정] `orchestrator.md`가 "co-plan은 gstack 스킬이라 직접 수정이 불가"라고 적고 있었다 — 틀렸다.** `skills/co-plan/SKILL.md`는 **repo SSOT**다(`sync-skills.sh` 동기화 대상은 grill-with-docs 하나뿐 = 자체 스킬은 미동기화, v3.11.0). 그 문면 때문에 co-plan 개선이 지금까지 **컨텍스트 주입 우회로만** 이뤄졌다. → "스킬 본문은 직접 고친다 / 주입은 **호출 시점에만 정해지는 값**(0단계 확정 rule 경로)에 한한다"로 정정.
- orchestrator 출력 활용 절: 기존 파일 수정이 포함되면 **`context`로 대상 파일 경로 전달**(Read 전제 — 안 주면 전건이 `⚠ 미확인` 라벨로만 나온다).

## 5.2.0 — 2026-08-14
- **회고 inbox 드레인 5파일 → 15 applied / wiki 4신설+1통합 / 3 rejected — MINOR. 재시작 권장.** (원후보 22건. authpatch 시간비용·BL-051·repostitch 위키미러·cross-branch 게이트제거·경로매핑 8a/8b)
- **⭐ [실측 수치도 자기보고다] orchestrator 자신의 결함 — 산문 주장과 수치 주장을 다르게 취급했다.**
  - 1회차 프로브의 `replaceAll(n)=n+5` **오측정**을 사실로 받아 "주석의 `2n+6`은 과대추정"이라 판정하고 테스트 계약 상수를 80,000→**150,000**으로 바꾸라 지시. 2회차 재측정 + codex 독립검증(9.5/10 일치): **`2n+6`이 맞았다.**
  - 결과: n=150,000이면 `replaceAll` 단독 300,006 > 예산 200,000이라 **독립/공유 무관하게 항상 INVALID** → 그 케이스의 판별력이 통째로 붕괴하고 신규 FAIL로 나타났다. **원래 값이 이미 정확했다.**
  - 뼈아픈 점: 이 세션은 서브에이전트에 *"자기보고는 검증 대상"·"분모를 요구하라"*를 **내내 강제**했다. 그런데 "실측했다"는 **숫자에는 그 잣대를 안 댔다**.
  - **정정**: 수치로 계약·상수·기대값을 바꾸려면 ① 독립 재측정 1회 또는 ② 기존 문서값과 불일치 시 **기존값 우위**(문서값은 이미 게이트를 통과한 것). 기존 주석·문서의 **산식을 뒤집을 때**는 교차검증 없이 진행 금지.
- **[grep 매치는 하한도 상한도 아니다]** 종전 문면은 *"grep 카운트는 하한이다"*(binary 오탐 무음제외)로 **누락 방향만** 경고했다. 같은 세션이 **양방향을 다 밟았다** — 과대 방향: `new GitProcessRunner()` 16건 매치를 "2파일 **확정 파손**"으로 위임 프롬프트에 박았으나 그 3건은 전부 순수 predicate만 리플렉션 호출하고 `buildProcess()`를 안 타 **fail-closed 게이트에 도달조차 안 했다**(무변경이 정답). ⚠ 서브에이전트가 **자발적으로 반박해서** 잡혔다 — 계약에 없는 행동이고, 순종적이었으면 무해한 3파일을 고쳤다. → 단정을 넣기 전 **매치마다 도달성 판정**, 안 했으면 "후보 — 도달성 미확인"으로 전달.
- **[R22 제어문자 리터럴 금지]** 7a가 계획서(SSOT)에 **raw 제어바이트를 주입**했다(자가신고 "3개" ↔ 실측 **4바이트**, 봉인도 불완전). 오염 파일은 ripgrep이 **binary로 무음 제외**해 이후 모든 인벤토리 grep이 조용히 실패한다 — `file`은 "UTF-8 text"라고 하는데 `grep`만 binary로 판정해 **자각 없이 지나간다**. 함정 자체는 `rules/…/frontend.md`에 이미 있었으나 **tester-design 계약엔 없어** 작성자가 규칙 존재를 모른 채 밟았다. 방지 지시 주입 후 **5배치 전부 오염 0** — 지시가 있으면 안 밟는다.
- **[승인 결정 반전의 문서 side]** 7c.2가 "값이 바뀌면 그 값을 단언하는 **테스트**를 전수 grep"은 강제하는데 **문서엔 같은 축이 없었다.** 폐기된 전제를 현행 계약으로 선언한 구 설계문서 3부작이 계획서 `## 수정 대상`에서 빠졌고 planner·설계패널 3인·codex 형제가 전부 놓쳐 **`/codex review` blocking에서야** 적발. 더 아픈 건 직전 기능이 majors F로 "SUPERSEDED 노트" 방침을 **이미 확정**해뒀는데 조치가 "finalizer 이월"로 적혀 **그 문서 안에서 죽고 다음 사이클 입력이 못 됐다.** → 반전 시 구 문서 전수 grep + 부분-SUPERSEDED 노트, "이월" 항목은 **순번 원장에 등재**해 다음 계획서의 필수 입력화.
- **[중간 산출 영속화]** 한 트랙에서 서브에이전트 3개가 한도로 죽었고 **전부 작업은 완주했는데 digest를 못 썼다**(마지막에 한 번 쓰는 구조). 지시를 넣고 발사한 건은 사망해도 회수됐고, 넣기 전 2건은 surefire 리포트·mtime을 뒤져 복원해야 했다. 지난 회차에 reject한 *복구 절차*(이미 존재)와 달리 이건 **예방 축**이라 채택.
  - 짝: **mtime·부분 grep은 완주 판정 근거가 아니다** — 그 근거로 "완료로 보임"이라 판정한 2건이 실제로 미해소였고 재발사 에이전트가 소스 추적으로 반증했다. 항목별 확인이 불가하면 **판정을 재발사 에이전트에 넘긴다**.
- **[병렬 발사가 실제로는 순차 — 비용 모델]** tmux 부재 환경의 foreground 고정 폴백을 **발사 방식으로만** 이해하고 배치 설계·비용 추정엔 반영하지 않았다. "병렬이니 싸다"로 배치를 쪼갰는데 실제는 **합산**. 실측: subagent duration 합산 ~14,000s 중 BE/FE 2배치 겹침 **0**(7.5 작성 BE 1069s + FE 980s = 2049s), codex(Bash background)만 실제로 겹쳤다.
- **[실DB 통합 재실행 연기]** 7.6/7.7 다회전이 같은 실DB 통합테스트를 **6회 이상** 반복(GREEN 자가검증 1057s · 변경검증 870s). 그 라운드 대부분은 순수 단위 결함만 고쳤다. → 재검증 라운드가 mock 기반 단위에 한정되면 `@SpringBootTest`는 최종 1회로 연기. ⚠ **orchestrator 명시 지시 한정 + 생략 클래스 반환 명시**(무음 부분실행 방어와 충돌 금지).
- **[codex probe exit124 단정 완화]** 유일하게 기존 규칙을 **약화**한 항목. 플러그인 preamble(PowerShell)이 60s 예산을 선점해 **정상 codex가 exit124로 죽었고** 180s 재시도는 exit 0이었다 — 종전 규칙대로면 멀쩡한 교차검증 소스를 버리고 단일소스로 격하했다. → `timeout 180` **1회만** 상향 재시도, 그래도 124면 불가 확정. 트레이드오프: stall PC 조기발각 최악 +120s ≪ 2소스 상실.
- **기타 applied**: 레이어 소유권도 §0④ 전제(계약 문구 주입 전 소유자 1회 grep — 7.7 critical 회수 1회) / delegation Read 트리거 ③ "실행·검증 요구 직전" + Bash 없는 에이전트 명시(`developer-*`·`tester-design`) / 워크스루 read-only 위임에 **실재 타입 명시**(`general-purpose`는 레지스트리에 없어 발사 실패) / 패널 **전멸=보존0 → 전체 재실행이 정상 갈래** + 패널 재런치가 codex 재호출을 함의하지 않음 / 백그라운드 에이전트 실행 중 워킹트리는 **과도상태**(1회 스냅샷으로 자기보고 반증 판정 금지 — `trap` 원복이 두 명령 사이에 발화해 판정 철회 1회) / 공유 실DB는 워크트리 분리로 안 풀림(자원 경합 축 확장).
- **[스크립트] `red-baseline.sh` SKIP_RE 오탐 제거**: 범용 `enabled\s*=\s*false`가 `spring.shell.interactive.enabled=false`(@SpringBootTest 규약 프로퍼티)에 걸려 워크스루 3.5에서 `skip마커 +1` 오탐. **규약을 지킬수록 오탐이 늘어** 진짜 `@Disabled` 신호가 묻힌다 → `@Test\([^)]*enabled\s*=\s*false`(TestNG 형태)로 축소. 회귀 sanity 실측 통과.
- **wiki**: 신설 4 — [[vitest-clearallmocks-once-queue]](`*Once` 큐 누출 = "단독 통과/전체 실패") · [[mock-substring-path-overmatch]](경로 부분문자열 mock 과잉적중 → **조용한 under-deletion**) · [[vitest-real-process-spawn-crash]](실 프로세스 대량 spawn 크래시, 회귀 아닌 부하) · [[shared-test-db-worktree-noop]]. 통합 1 — [[codex-bash-direct-timeout]]에 **sentinel 부재 ≠ 죽음**(래퍼 sh만 죽고 codex.exe 생존 → `tasklist` + 로그 size 증가로 판정).
- **rejected 3건 — 전부 "이미 존재"로 실측 반증**:
  - 선행 드롭 6건이 "이동만 되고 규칙화 안 됐을 가능성" → v5.0.0에 **전건 적용**(`playbook-tdd:154` 부정단언 · tester-frontend "첫 행동은 실행" · `wiki/sources/` 사본). 회고가 그 커밋 전에 쓰였다.
  - `@Nested` 무음 스킵 → `tester-backend` :27·:65·:68·:99 + [[surefire-nested-skip]]. 회고 본문도 *"전부 tester가 스스로 잡아 재실행"* = **규칙이 작동한 기록**이다.
  - JLine dumb-terminal → `tester-backend:37-41` + `tester-runtime:24` + [[springboottest-jline-terminal-hang]], 플래그까지 동일. **규칙 갭이 아니라 미준수** — 또 쓰면 중복만 는다.
- ⚠ **A축 부채 명시**: 이 배치로 `orchestrator.md` **960→967줄 / 12,517→13,073단어**(+556). 8건을 playbook·wiki로 라우팅해 억제했으나 순증은 남았다. 감축 게이트는 아직 없다 — 별도 안건.
- ⚠ WARN: `sync-skills.sh` `⛔ 대조된 스킬 0개`(grill-with-docs 상류 부재) — **5회 연속**.
