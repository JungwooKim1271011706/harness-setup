...
## 하네스 자체가 범인인지 이분 격리 (`--safe-mode`)

훅·스킬·session-check가 이상 동작할 때, **하네스 커스터마이징이 원인인지 환경 문제인지**를 먼저 가른다: `claude --safe-mode` 세션(모든 `.md`/훅/스킬/플러그인/MCP OFF)과 대조한다. safe-mode에서 재현되면 하네스 무관(환경·CC 자체), 안 되면 하네스가 범인.

- 코드 변경 0·거버넌스 무영향. 차단 훅 5종 + Stop/SubagentStop 훅이 겹쳐 표면을 가르기 어려울 때 1회 대조가 싸다.
- ⚠ 만능 아님: **로직 갭**(규칙은 있는데 메커니즘이 없는 클래스)은 safe-mode로 안 잡힌다 — 그건 훅이 오작동하는 게 아니라 애초에 없는 것이다. 이 절차가 쓸모 있는 건 "훅/스킬이 이상하게 동작한다" 류뿐이다.
