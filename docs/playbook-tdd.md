...
- **승인 결정 반전 (문서 side — 코드가 아니라 문서가 stale해지는 축)**: 이번 변경이 **기존 ADR 결정·승인된 설계문서의 계약을 반전**하면, 그 결정을 **기재한 모든 문서**(구 feature 문서·ADR·CONTEXT)를 전수 grep해 **수정 대상에 넣는다**. grep 키는 코드 심볼이 아니라 **폐기되는 전제 문장·상수명·모드명**이다. 조치는 본문 재작성이 아니라 **상단 부분-SUPERSEDED 노트 + 구 계약 표 인라인 캐비어트**(역사 기록 보존). ⚠ 코드 계약은 테스트가 지키고 리뷰가 보지만 **문서 계약은 아무도 안 본다** — 다음 세션의 planner가 stale SSOT를 읽고 틀린 baseline으로 계획한다. 근거: 직전 기능 설계문서 3부작이 폐기된 전제를 **현행 계약으로 선언한 채** 남았고 계획서 `## 수정 대상` 12행에 없었다 — planner·설계패널 3인·codex 형제(플랜단계) 전부 놓치고 **`/codex review` blocking에서야** 잡혔다. 더 뼈아픈 건 직전 기능이 majors F로 이미 "SUPERSEDED 노트 방침"을 확정해뒀는데 조치가 "finalizer로 이월"로 적혀 **그 문서 안에서 죽고 다음 사이클 입력이 못 됐다**(2026-08-14). → **"이월" 딱지 항목은 feature 문서 안에만 두지 말고 순번 원장(placeholder)에 등재**해 다음 계획서의 필수 입력으로 만든다.
...
- **시그니처/위임 전환**(호출처 + 사라지는 책임): ① **생성자/메서드 시그니처 변경**(필드·파라미터 추가/DI 변경) → 그 생성자·메서드를 호출하는 **모든 기존 테스트** grep(컴파일 깨짐·stale setup 사전식별). ② **기존 호출경로를 다른 컴포넌트로 위임/대체 전환** 시, 대체되는 메서드가 수행하던 **부가책임**(예: `resolveTargetFiles`의 JAR 다중배포 확장)을 신경로가 보존하는지 인벤토리 — 위임이 "주 동작"만 옮기고 부가책임을 빠뜨리는 회귀를 RED로 선잠금(안 하면 변경검증 GREEN 후 review서야 blocking 적발). 근거: AutoPatchCommands 생성자 2필드 추가 stale 마이그레이션 라운드 + resolveTargetFiles JAR확장 책임소실 codex review blocking(2차 수정). ⚠ **grep 매치는 후보 목록이다 — 하한도 상한도 아니다.** ① **빠질 수 있다(하한)**: ripgrep이 비-UTF8 바이트 섞인 파일을 binary 오탐해 **무음 제외**한다([[grep-binary-misdetect-touch-surface]], WI-C 11≠12파일 실측) → `grep -IL . -r <트리>` 병행, 최종 봉인은 `mvn test-compile` 1회(컴파일이 진실). ② **영향을 뜻하지 않는다(상한)**: 호출은 있는데 **대상 분기에 도달하지 않을 수 있다** → 위임 프롬프트에 "확정 파손"·"전부 깨진다" 류 **단정을 넣기 전 매치마다 도달성을 소스로 판정**한다. 판정 안 했으면 단정 대신 **"후보 — 도달성 미확인"**으로 전달한다. 근거(상한): `new GitProcessRunner()` 16건 매치를 "2파일 확정 파손"으로 옮겨 지시했으나, 그 3건은 전부 `exportConfig` 미참조 순수 predicate만 리플렉션 호출하고 `buildProcess()`를 안 타 fail-closed 게이트에 **도달 자체를 안 했다** — 무변경이 정답. 서브에이전트가 자발적으로 반박해 잡혔다(계약에 없는 행동, 순종적이었으면 무해한 3파일을 고쳤다). 2026-08-06.
...
- **재검증 라운드의 실DB 통합테스트 연기 (라운드 단가 절감)**: 7.6/7.7 **재검증** 라운드에서 수정 범위가 **순수 단위 케이스(mock 기반)에 한정**되면 실DB `@SpringBootTest` 클래스는 **최종 1회로 미루고** 그 라운드는 단위만 돌린다. **계약·스키마·배선을 건드린 라운드는 전량 필수**다. ⚠ **축소는 orchestrator가 명시 지시할 때만이고 tester 재량이 아니다** — 생략한 클래스는 반환에 명시해 무음 부분실행(`tester-backend.md` 지시↔실행 스코프 대조)이 되지 않게 한다. 근거: 7.6/7.7 다회전에서 같은 통합테스트를 **6회 이상 반복 실행**(GREEN 자가검증 1057s · 변경검증 870s · developer 보고 10:47min). 그 라운드 대부분은 단위 결함만 고쳤고 실DB가 검증한 내용은 직전 라운드와 동일했다. 1줄 수정 검증에 7클래스 전량 + 실DB 2기동으로 268~716s(2026-08-04).
- 처리:
  - 통과 → 7.7 진행.
...
**currently-PASS 케이스 3분류 (필수 — FAIL 축과 별개)**

위 2분류는 **FAIL한 케이스**만 본다. 그런데 tautological 케이스는 **FAIL하지 않는다 — PASS한다.** 그래서 판정표에 아예 걸리지 않는다. 그리고 PASS 자체는 결함이 아니다 — 정당한 negative 대조군과 회귀가드도 GREEN 전에 PASS한다. **셋을 구분하는 축이 없었다.**

1. 작성자(7.5)는 반환 계약에서 **"RED가 아니라 이미 PASS할 것으로 보이는 케이스"를 지목**한다(선택 → **필수**).
2. 7.6이 그 목록을 받아 각각을 3분류한다:

   | 분류 | 뜻 | 판정 |
   |---|---|---|
   | (a) 정당한 negative 대조군 | 잘못된 구현이면 **FAIL로 뒤집힌다** | 통과 |
   | (b) 회귀가드 | 특정 회귀에서만 FAIL — 성격이 **제목·주석에 명시**돼야 한다 | 통과 |
   | (c) **tautological** | **어떤 구현에서도 결과 불변** | **1건이라도 있으면 7.6 불통과** → 작성자 반환 |

3. **판정 방법**: 그 케이스의 단언 결과를 **목 입력이 전부 결정하는가**를 본다. 결정하면 (c).
4. **예측 카운트 명시**: 교정 후 재실행 시 "몇 건이 어느 방향으로 이동할지"를 **먼저 적고** 실행으로 검산한다. 실측에서 `30 failed → 31 failed`로 정확히 1건 이동해 **판별력과 부수 파손 0을 동시에 증명**했다.

> 근거: 목이 변환 **후** 이름만 담아 구현 필터가 항상 배제하고 `diff.stdout`을 `''`로 하드코딩한 케이스가 **7a·7b·7.5·7.6을 전부 통과**했다 — 그 RED 스위트가 기각하려는 **바로 그 구현(가드 부재)도 통과**시키는 상태. 7.7의 2검증자만 잡아 교정 라운드 1회 추가. 이번엔 7.6이 **이미 감지는 했는데 차단 권한이 없어 비차단 관찰로 흘려보냈다** — (c)에 차단 권한을 주는 것이 이 절의 핵심이다(2026-09-06).
...
| 작성자 | 검증자 | 교차 원칙 |
|--------|--------|---------|
| codex (7.5 정상) | tester-quality(claude — **fable 1순위**. 미가용은 무음 강등이라 자동 폴백 없음 → orchestrator `### 모델 실측`으로 확인 후 opus 재스폰) 호출 | 작성자≠검증자 |
...
### 8.0 위임 커버리지 대조 (배치 확정 직후·발사 **전**, 필수)

배치를 나눈 뒤 **위임을 쏘기 전에** orchestrator가 1:1 매핑표를 만든다. 미매핑 항목이 1건이라도 있으면 **위임 발사 금지**.

| 계획서 승인 항목 | 출처 | 배치 | 상태 |
|---|---|---|---|
| (항목) | `### 수정 대상` / `영향범위` / `승인된 FROZEN 예외` | A/B/… | 매핑 |

- **왜 여기인가**: 기계 게이트는 전부 **있는 코드**만 본다 — 7.7=작성된 RED 품질, 변경검증=실행된 테스트, /review·codex review=diff, /cso=존재하는 공격표면. **있어야 하는데 없는 코드**는 어느 게이트도 못 본다. 미구현 승인항목은 전 게이트를 **무사통과**한다.
- **7c.1과 짝**: 7c.1은 "승인 major → 최소 1 RED"를 강제한다. 8.0은 "**승인 수정대상 → 최소 1 위임 배치**"를 강제한다. 종전엔 후자만 강제가 없어 비대칭이었다.
- **오기억 방지**: 승인 항목은 **기억이 아니라 계획서를 다시 Read해서** 열거한다. 근거 사고의 뿌리가 orchestrator의 오기억이었다(승인 예외 2건을 다른 항목으로 잘못 기억 → 배치 지시에서 1건 누락).
- 최종 그물은 워크스루 3단계 양방향 대조(`orchestrator.md`)다. 8.0은 **구현 전에** 잡아 재작업을 없앤다.
- 근거: 승인된 FROZEN 예외 ①(소켓 타임아웃 프로퍼티화)이 미구현인 채 7.7·변경검증 77/77·/review·/codex·/cso 전부 통과, finalizer 직전에야 발각(2026-07-27). 미조치 시 hang 소켓이 공유 풀 슬롯 영구 점유 → pool 포화 후 전 요청 BUSY.

### 8.0b RED 기준선 스냅샷 (커버리지 대조 통과 직후·발사 **전**, 필수)

`bash .claude/scripts/red-baseline.sh snapshot` 1회. 현재 테스트 파일 전부를 git blob으로 고정한다(출력 1줄).

- **왜 여기인가**: 7.6/7.7을 통과한 이 시점이 **RED 기준선**이다 — 단언이 "올바른 이유로 FAIL"함이 검증된 유일한 지점. 이후 GREEN·FAIL 3분기 루프에서 테스트가 바뀌면 이 스냅샷이 유일한 비교 대상이 된다.
...
- ⚠ **스냅샷 직후 "이번 기능 테스트가 실제로 포함됐는지" 1회 확인한다(grep 1회).** 출력의 **모듈 분포** 줄에서 이번 변경 모듈이 `0`이면 수집 범위가 틀린 것이다. 스크립트는 v5.9.0에서 ignored 테스트도 수집하도록 고쳤지만(`.gitignore`에 `*/src/test/`를 둔 프로젝트에서 전량 누락됐었다), **"대상 0"과 "이탈 0"을 사람이 혼동하는 축은 남는다** — 총계만 보고 넘어가지 마라. 실측: "61파일 고정" 보고가 전부 타 모듈이었고 이번 RED 6파일은 0건이었다(2026-09-07).
