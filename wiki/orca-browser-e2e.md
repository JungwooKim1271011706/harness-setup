...
## 플랫폼 조건 — macOS면 Aside가 같은 자리를 채운다

gstack v1.84+ 는 **Aside** 브라우저를 1순위 드라이버로 쓴다. Aside도 **사용자의 실제 로그인 세션**을 그대로 타므로 위 "최대 이점"이 동일하게 성립한다.

⚠ **단 Aside는 macOS 15+ 전용이다.** `browse/SKILL.md`가 `uname -s`가 `Darwin`일 때만 Aside를 권고하고, **Windows·Linux에서는 gstack 자체 헤드리스 브라우저로 폴백**한다(로그인 세션 공유 없음 — 쿠키를 따로 넣어야 한다).

| 플랫폼 | 실 로그인 세션으로 렌더 실측 |
|---|---|
| macOS 15+ | gstack **Aside** (`/browse`·`/qa`가 자동으로 탄다) |
| **Windows · Linux** | **Orca** — 이 페이지의 경로가 사실상 유일하다 |

`scripts/browser-probe.sh`가 **Orca를 먼저** 찾고 없으면 gstack `browse`로 떨어지는 순서인 이유가 이것이다. 맥 사용자에게는 Orca 대신 `/browse`를 권해도 된다.
