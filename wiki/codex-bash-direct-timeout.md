...
sources:
  ...
  - sources/20260813T042455Z-repostitch-wiki-mirror-sync.md (래퍼 sh만 죽는 오펀 — sentinel 부재 오독)
updated: 2026-08-14
---
...
- ⚠ **sentinel 부재를 "죽었다"로 읽지 마라 (래퍼 sh만 죽는 케이스)**: `nohup sh -c "codex exec ...; echo sentinel"` 패턴에서 세션 teardown이 **래퍼 sh만** 죽이면 codex.exe는 살아남아 리다이렉트된 fd로 **로그를 계속 쓰지만 sentinel은 영원히 안 찍힌다**. 생존 판정은 sentinel이 아니라 **`tasklist`의 프로세스 생존 + 로그 파일 size 증가** 두 축으로 한다. 실측: 한 배치가 정확히 이 상태였고 bounded 폴링으로 완주 회수했다(exit 0, 산출 온전 — 2026-08-13). 폴링 함정 일반론은 [[codex-background-polling-traps]].
