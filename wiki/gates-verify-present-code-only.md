...
sources:
  ...
  - ../CHANGELOG.md — v4.3.0 "RED 기준선 대조" (약화 축 추가)
updated: 2026-07-28
---
...
**같은 사각의 셋째 얼굴 — 부재 말고 *약화* (v4.3.0 추가):**

표는 "없는 코드"를 못 본다는 축만 적었는데, **약해진 코드**도 같은 사각이다. 기계 게이트 중 **테스트 최종본의 단언 강도**를 보는 것이 하나도 없다:

| 게이트 | 왜 못 보나 |
|---|---|
| 7.7 tester-quality | GREEN **전**에 돈다 — 구조상 최종본을 못 본다 |
| `/review`(code-reviewer) | 핵심규칙이 *"7.5 RED 테스트 자체는 대상 아님"* — **명시적 제외** |
| `block-developer-test-edit.sh` | developer의 **편집 행위**만 막는다(구문 경로). 작성자 재작성 루프는 정당 경로라 안 걸린다 |

→ **3. RED 기준선 대조**(`red-baseline.sh`, 8.0b 스냅샷 ↔ 워크스루 3.5 대조): 7.6/7.7 통과 시점의 테스트 blob을 고정하고 finalizer 직전에 대조. 탐지는 스크립트(결정론), 약화 vs 정당한 교정 **판정은 LLM**. 스냅샷이 없으면 `⚠ 미검증`을 출력한다 — **부재를 통과로 읽지 않는 게 이 페이지의 교훈 그 자체**라 무음 통과를 구조적으로 막았다.
