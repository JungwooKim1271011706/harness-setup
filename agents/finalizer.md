...
🔍 사람 E2E 점검 (비차단 — 읽고 직접 확인, 커밋은 진행됨)
  ...
  ⏱ 소요 ~<N>분  ·  🧰 필요: <dev server 기동 / 로그인 계정 등>
  ✅ 자동 커버 (다시 안 봐도 됨): <tester 변경검증 PASS 항목>

  [준비] (1회성)
    - <의존 기동·시드 — 없으면 생략>
  [빌드/실행]   (CLAUDE.md Build & Run 인용 — 그대로 복붙)
    $ <정확한 빌드/실행 명령>
  👁 사람 E2E 필요 (oracle: <feature 문서 | 원 요청>):
    | # | 무엇을 한다 (클릭/입력 대상) | 기대 결과 (✅ 성공 신호) | ✗ 실패 힌트 | 통과? |
    |---|------------------------------|--------------------------|-------------|-------|
    | 1 | <화면/경로 — 자동 추출>로 이동  | <화면이 떠야 함> ✅        | <✗ 안 뜨면 → 흔한 원인> | ☐ |
    | 2 | <버튼/링크 — 자동 추출> 클릭    | <기대 변화 — oracle 기준> | <✗ … >       | ☐ |
  (feature 문서 있으면 동일 내용을 `## 수동 E2E 검증`으로 append됨)
```
