...
- 추측 금지, 근거 부족 시 "미확정"
- **다국어·장문 커밋 메시지는 heredoc으로 넘기지 않는다 — 파일 + `git commit -F <파일>`.** Bash heredoc에 긴 한글 텍스트를 넣으면 깨진다(직전 세션 2회 실패). Write 도구로 메시지 파일을 만든 뒤 `-F`로 읽힌다. 실효 확인: 그 방식으로 커밋한 뒤 `git log -1 --format=%B`와 원본 파일이 **끝 개행 외 차이 0**(2026-09-11). 코덱스 경유 프롬프트의 메타문자 파손과는 별개 축이다([[codex-bash-heredoc-metachar]]).
- **`git rerere`가 충돌을 조용히 자동 해결한다 — 대조 전 `git add` 금지.** rebase·merge 위임 전에 `git config rerere.enabled`를 확인한다. 켜져 있으면 rebase가 충돌 파일을 *previous resolution*으로 채우고 **충돌 표시 0 · unmerged 상태**로 멈춘다 — **누가 언제 남긴 해결인지 모른다**. 자동 해결된 파일은 반드시 **확정 해결안과 바이트 대조**한 뒤에만 `git add` 한다. 실측: 대조해서 동일함을 확인하고 진행했으나, 안 했으면 **출처 불명 해결이 검증 없이 커밋**됐을 것이다(2026-09-11).
- **브랜치·워크트리 작업 중에는 제품 repo의 순번 원장(`docs/backlog.md` 등 "마지막 번호+1" 관례 파일)을 수정하지 않는다.** 등재는 **머지 후 base 브랜치에서만** 한다. 번호 공간이 브랜치별로 분기해 append 지점이 겹치면 머지 충돌이 **구조적으로 재발**한다(파일 끝 append + 병렬 워크트리 = 확정 재발). 근거: 브랜치가 `BL-027`을 등재하는 사이 main도 `BL-027`/`BL-028`을 등재 → `git merge` CONFLICT → 충돌 해소 + 리넘버링 1라운드. 같은 세션 2차 라운드에서 원장을 안 건드리자 **충돌 0**으로 실증(2026-07-30). 브랜치 중 등재가 꼭 필요하면 번호 대신 충돌 없는 ID(날짜+슬러그)를 쓴다.
  - **등재 시 placeholder → 실번호 일괄 치환**: planner가 계획서·ADR에 남긴 `BL-NEW-n`(planner 핵심규칙)을 실번호로 바꾸되 **계획서·ADR·소스/테스트 주석까지 grep 1회로 전수** 치환하고 잔존 0을 확인한다.
  - **append 전 번호를 소스 주석에 박지 않는다.** 주석·테스트·ADR에 인용된 번호는 사후 리넘버링을 사실상 불가능하게 만든다. 근거: 유입 브랜치가 번호를 소스 3파일 + 문서 ~25곳에 이미 인용해 옮길 수 없어 **HEAD 측 9건(040~048 → 042~050)을 대신 리넘버링**(상호참조 6곳 동반)했다. 다른 트랙 계획서가 같은 대역을 예약해 3번째 충돌이 예고된 상태였다. 2026-08-04.
