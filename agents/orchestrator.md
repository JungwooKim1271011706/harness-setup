...
픽스·구현의 correctness가 **외부·런타임 데이터**(kit 매니페스트 필드·repo config·API 응답·파일 내용·서버/브랜치 default 값 등)에 의존하면, planner·설계패널·TDD 진입 **전에** orchestrator가 **실 샘플 ≥1개를 직접 inspect**(Read/git/grep)해 그 전제를 확정한다.
