...
- **병렬 발사 직후 선언 ↔ 실제 tool_use 1:1 대조 (declared-but-not-dispatched 차단)**: 한 메시지에 여러 도구를 병렬로 보냈으면, **선언한 목록과 실제 보낸 tool_use 블록을 세어 대조**한다. 특히 **`/review(code-reviewer) ∥ /codex review`는 교차검증 2소스 불변식**이라 **한쪽 누락 = 단일소스 격하**인데, 누락을 자동 탐지하는 장치가 없다. 실사고: "리뷰 3종 병렬(code-reviewer ∥ codex review ∥ design-reviewer)"이라 선언하고 실제로는 Bash + code-reviewer + design-reviewer를 보내 **`/codex review`가 빠졌다** — 다음 턴에 자각해 별도 발사, 라운드 1회 추가(2026-08-04). 자각이 늦었으면 단일소스로 종결됐을 것이다. 누락 발견 시 즉시 보충 발사하고, 산출에는 **병렬이 아니라 순차였음**을 기록한다(교차검증 독립성은 유지된다).
...
1. **흐름 서사**: 엔트리포인트부터 데이터 흐름 — 전부 `file:line` 링크. 예: "요청 → `XController:45` → `YService:120` 검증 → `ZRepo:60` 저장, 실패 시 `:88` 폴백"
