...
- **테스트파일 편집 위임 후 디스크 반영 검증 (claimed-but-not-applied 차단)**: tester-design 등에 테스트파일 작성·재작업을 위임한 산출을 받으면, **다음 단계(7.6/변경검증) 진입 전에** 보고된 핵심 변경 1~2개를 orchestrator가 직접 `Grep`로 디스크 확인한다. 보고 텍스트만 믿고 진행 금지 — Edit/Write 누락·실패·오위치로 보고≠디스크가 3+회 재발(PR-F2 픽스처 미반영, PR-C2 파일 자체 부재 2회). 불일치면 즉시 재발행(tester-design 산출에 grep 자가증명이 포함됐으면 그걸로 갈음 가능). 비용 grep 1회 << 변경검증 1라운드.
...
- 적용 지점: 설계패널 생존/모순 critical 에스컬레이션, 7c 상호배타(`playbook-tdd.md`), FAIL 3분기(설계결함→재승인·환경문제→가이드), 3루프 ESCALATION 등 **모든 사용자 판단 위임**.
