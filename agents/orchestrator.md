...
- **세션 종류 분기**: dev clone은 직접 wiki 커밋. **소비자 세션은 직접커밋 금지 — 회고 inbox로 드롭**(개선후보와 동일 운반; 판별식·경로·형식·드레인은 `wiki/_schema.md` "어디로 가나" 절 SSOT — `basename $(git rev-parse --show-toplevel)`=`.claude`면 소비자. origin 판별 금지=오판). retro가 dev clone에서 드레인해 SSOT에 안착.
