...
- 백엔드 전용: planner-backend -> 설계패널게이트(≥3, ∥ codex 형제) -> 승인 -> TDD합의(7a∥7b→7c→7.5→7.6 RED sanity→7.7품질게이트) -> developer-backend -> tester-backend -> /verify-implementation(verify-* 스킬 등록 시) -> /review(code-reviewer) ∥ /codex review(병렬) -> /cso(인증/권한/암호화 변경 시) -> finalizer
