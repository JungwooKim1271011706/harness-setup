...
- **상호의존 작업 병렬위임 금지 (의미적 의존 ≠ 파일 충돌)**: Agent 도구로 두 작업을 **한 메시지에 동시 발사**하기 전, "**한쪽 산출이 다른쪽 입력 가정을 바꾸나?**" 1문 자가체크. 바꾸면 **순차**다. 특히 **구현 계약 변경 ↔ 픽스처/테스트 모사**는 상호의존 — developer가 정하는 접점계약(argv 형태·`-C` 부착·메서드 선택)을 tester-design 픽스처가 모사하므로, **구현 먼저 확정 → 접점계약 SSOT 못박고 → 픽스처 정합** 순. "developer=src·tester=test라 파일 비충돌 → 병렬 가능"은 함정(파일 비충돌이 의미 독립을 뜻하지 않음). 병렬은 산출이 서로의 전제를 안 바꾸는 독립 작업만(예: 무관 모듈 2개, 패널 ∥ codex). **신규 공유 타입/DTO도 동일**: 2저자 병렬 저작(tester-design ∥ codex 등) 발사 전 orchestrator가 **필드명 + 스텁 형태를 1회 freeze해 양 저자에 주입**한다(접점계약 freeze의 타입판) — 미확정 병렬 시 필드명 divergence·고아 케이스·스텁 불균일.
  - **자원 경합 축 (의미 독립 ≠ 자원 독립)**: 위 1문을 통과해도 "두 작업이 **같은 빌드 산출 디렉터리(`target/`·`dist/`·`node_modules/.vite`)·포트·DB·락파일**을 쓰나?" 1문을 더 자가체크한다. 쓰면 **순차**다. 근거: 같은 모듈 tester 배치를 병렬 발사했다가 Maven `target/` 경합으로 순차 전환 — 병렬 이득이 통째로 소멸(2026-08-04). ⚠ `isolation: 'worktree'`가 해법으로 보이지만 워크트리는 **별도 체크아웃**이라 미커밋 산출이 따라가지 않는다 — GREEN 직후 tester 검증처럼 검증 대상이 미커밋인 단계엔 쓸 수 없다. 커밋된 상태만 보는 작업에서만 고려. ⚠ **공유 실DB는 워크트리 분리로도 안 풀린다** — `@ResourceLock`·`SAME_THREAD`는 **프로세스 간에는 무효**라 mvn 2프로세스 동시 실행이 `DuplicateKeyException`·빈 누락 등 산발적 DB 오염을 낸다. 테스트 DB가 하나면 자원 축이 워크트리보다 위다 → 순차만이 답([[shared-test-db-worktree-noop]]).
...
**1~4단계 — 브리핑 (서브에이전트 위임 생성)**: fresh read-only 서브에이전트(Read/Grep/Glob)에 위임한다 — ⚠ **타입은 레지스트리에 실재하는 것으로 지정**한다(read-only 용도 = `code-reviewer`, 프롬프트에 "리뷰 아님 · 지도 그리기"를 명시). `general-purpose`는 Agent 도구 설명상 기본값이지만 **이 하네스 레지스트리엔 없어 발사가 실패**한다(2026-08-13 실측 — 재발사 1회) — diff 전문이 메인에 실리지 않게(`## 산출 수신 계약`). 반환 ≤30줄, 사용자에게 그대로 제시:
