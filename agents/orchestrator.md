...
- 프론트 전용: planner-frontend -> 설계패널게이트(≥3) -> [디자인 목업 게이트: UI태그+신규화면 시 /design-shotgun→/design-html] -> 승인 -> TDD합의(7a∥7b→7c→7.5→7.6 RED sanity→7.7품질게이트) -> developer-frontend -> tester-frontend -> [design-reviewer: 목업 게이트 발동 시] -> [/qa-only: UI태그+servable URL 시, report-only→developer-frontend] -> /verify-implementation(verify-* 스킬 등록 시) -> /review(code-reviewer) ∥ /codex review(병렬) -> /cso(인증/권한/암호화 변경 시) -> finalizer
...
- 혼합/고복잡도: planner-high-complexity -> 설계패널게이트(3~4 연관기반, eng 다라운드가 아키텍처 검증 흡수) -> [디자인 목업 게이트: UI태그+신규화면 시 /design-shotgun→/design-html] -> 승인 -> TDD합의(7a∥7b→7c→7.5→7.6 RED sanity→7.7품질게이트) -> 도메인별 developer/tester 분리 -> [design-reviewer: 목업 게이트 발동 시] -> [/qa-only: UI태그+servable URL 시, report-only→developer-frontend] -> /verify-implementation(verify-* 스킬 등록 시) -> /review(code-reviewer) ∥ /codex review(병렬) -> /cso(인증/권한/암호화 변경 시) -> finalizer
...
**live-UI 자동 QA 짝 (`/qa-only`)**: UI 트랙(프론트/혼합)에서 tester-frontend PASS 직후, **servable URL이 확보되면** gstack `/qa-only`(report-only)로 실기동 UI를 클릭스루 자동 점검한다. design-reviewer와 달리 qa-only는 **이미 report-only라 스킬을 직접 사용**한다(`/qa` 자동수정 변형은 거버넌스 충돌이라 금지 — report-only만). 발견 = `/review` 발견과 동일 취급: 명백한 flake는 필터, 실버그는 **developer-frontend 재작업**(수정분만 tester 재검증). 목적 = 자동 JUnit이 못 보는 실기동 UI 표면을 자동으로 메워 **finalizer 사람E2E 점검표를 축소**(회귀 oracle 이원화의 사람 몫 일부 자동화). **URL 부재·앱 미기동·gstack 미설치 = 스킵**(비차단, 사람E2E가 커버 — "L2 풀 런타임은 무겁고 환경의존"이라 best-effort). URL 출처: `review/scenarios.local.md`의 기동 URL 또는 tester-frontend 기동 산출. 라우팅표 "구현 결과 live-UI 자동 QA" 행 참조.
...
| 구현 결과 live-UI 자동 QA (tester-frontend PASS 후) | `/qa-only` (gstack, report-only) | UI 태그(프론트/혼합) + servable URL 확보 시 (실기동 클릭스루. 발견→developer-frontend, URL부재=스킵·비차단. `/qa` 자동수정 변형 금지) |
