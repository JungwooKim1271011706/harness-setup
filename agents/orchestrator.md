...
- **편집 위임 후 디스크 반영 검증 (claimed-but-not-applied 차단)**: tester-design 테스트파일뿐 아니라 **developer의 빌드계약 변경(pom/assembly/descriptor)**도 동급 — 작성·재작업 산출을 받으면 **다음 단계(7.6/변경검증) 진입 전에** 보고된 핵심 변경 1~2개를 orchestrator가 직접 `Grep`로 디스크 확인한다. 보고 텍스트("BUILD SUCCESS"·"적용함")만 믿고 진행 금지 — Edit/Write 누락·실패·오위치로 보고≠디스크가 3+회 재발(PR-F2 픽스처 미반영, PR-C2 파일 부재 2회, pom assembly 변경 거짓보고 1회). ⚠ developer가 **compile 자가확인만** 한 빌드계약 변경(assembly descriptor 등)은 `mvn compile`이 안 잡는다(package 영향) → 변경검증서 실 `mvn package` 1회 또는 핵심 라인 grep. 불일치면 즉시 재발행(산출에 grep 자가증명 포함됐으면 갈음). 비용 grep 1회 << 변경검증 1라운드.
- **상호의존 작업 병렬위임 금지 (의미적 의존 ≠ 파일 충돌)**: Agent 도구로 두 작업을 **한 메시지에 동시 발사**하기 전, "**한쪽 산출이 다른쪽 입력 가정을 바꾸나?**" 1문 자가체크. 바꾸면 **순차**다. 특히 **구현 계약 변경 ↔ 픽스처/테스트 모사**는 상호의존 — developer가 정하는 접점계약(argv 형태·`-C` 부착·메서드 선택)을 tester-design 픽스처가 모사하므로, **구현 먼저 확정 → 접점계약 SSOT 못박고 → 픽스처 정합** 순. "developer=src·tester=test라 파일 비충돌 → 병렬 가능"은 함정(파일 비충돌이 의미 독립을 뜻하지 않음). 병렬은 산출이 서로의 전제를 안 바꾸는 독립 작업만(예: 무관 모듈 2개, 패널 ∥ codex). 근거: PR-F3 developer(judgeFf `-C tmpDir` 부착) ∥ tester-design(`-C` 없음 가정) 동시발사 → 정반대 가정 → 변경검증 8→**57 FAIL** 폭증 + 재진단 1라운드. **신규 공유 타입/DTO도 동일**: 2저자 병렬 저작(tester-design ∥ codex 등) 발사 전 orchestrator가 **필드명 + 스텁 형태를 1회 freeze해 양 저자에 주입**한다(접점계약 freeze의 타입판) — 미확정 병렬 시 필드명 divergence(fromRef vs fromSha)·고아 케이스·스텁 불균일(web-export 2026-07-21).
...
- 게이트(패널·TDD·리뷰)는 픽스의 **내부 정합성**만 검증한다 — 픽스가 딛고 선 **외부 데이터 전제 자체**는 아무도 안 본다. 전제가 틀리면 전 게이트를 통과한 **오진이 shipped**되고 라이브 E2E에서야 발각(완전 1사이클 낭비). 근거 추가: WI-C 인라인 L1 — 존재검증이 소스경로 기준(실제=배포경로+`.class` 변환)이고 픽스처도 동형 모사 → 전 게이트 GREEN, 라이브만 발각(tester-design R17 짝, 2026-07-20).
...
- 근거: repostitch bb1f564(오진 전 게이트 통과→라이브 발각)→7ef9cb9(정식). 픽스가 "매니페스트 default=main" 전제였으나 실 kit `defaultBranch=amd-test`라 방향 무의미. 실 샘플 1개 Read면 진입 전 발각됐다. + tracker-migration(mock RED 전 게이트 통과 → Gitea 11함수 전량 404·GitLab PATCH/assignee 실패, cross-model+curl 실측만 적발. "Gitea 계약을 GitLab에 복사"를 어떤 게이트도 안 봄).
...
`Agent(run_in_background:true)`가 `tmux requires WSL` 류로 즉시 실패하는 환경(Windows git-bash, tmux 부재)에서는 **재시도 없이 Agent는 foreground 고정**한다. 병렬이 필요하면 **Bash background(codex 등) ∥ Agent foreground 조합**으로 구성한다. 근거: WI-B 7a(bg 실패→fg 재발사로 병렬성 상실) + review 단계 codex Bash bg ∥ code-reviewer fg 우회 성공 실측(2026-07-20).
...
3. **생존 critical > 0** → 게이트 차단, planner 재작업 [LOOP n/3].
   **생존 critical == 0** → 사용자 승인 단계로. `majors` 승인화면 노출, `minors` 기록.
   ...
   - **파생 문서 동기화 (패널 major/critical로 결정 번복 시)**: 패널 지적으로 grill 단계 결정이 뒤집히면(예: 502→500), 그 결정이 기재된 **프로젝트 지식 문서(`CONTEXT.md` 용어·`docs/adr/`)를 같은 라운드에서 갱신**한다. orchestrator.md "분리 문서 drift 방지"는 harness 문서(playbook)만 커버하고 CONTEXT/ADR 번복 동기화는 규칙이 없었다 — 계획·ADR만 고치고 CONTEXT 용어집을 빠뜨리면 틀린 용어가 커밋된다(finalizer 코드대조가 백스톱이나 게이트가 앞단에서 잡아야). 근거: trackA 트리거 미설정 502→500 번복 시 CONTEXT.md 용어 갱신 누락(finalizer가 `WebExportService.java:108`로 잡음).
...
**검증단계 짝**: 이 게이트가 발동했으면 **또는 렌더 중심 기능(SVG/canvas/신규 시각화 — 목업 게이트 미발동이어도)이면** 구현 후 tester-frontend PASS 직후 **design-reviewer 서브에이전트(report-only)**가 목업↔구현 드리프트(목업 있을 때) + 디자이너 시선 폴리시·시각 불변식을 본다(발견→developer-frontend). 렌더 기능 확대 근거: web-export SVG 스타일 유예 blocking 6이 이 리뷰서야 발각(2026-07-21, tester-design R18의 백스톱 짝). gstack `/design-review` 스킬의 fix-loop는 거버넌스 충돌(소스 자동수정·커밋)이라 안 쓰고 **루브릭만 재사용**한다. 라우팅표 "구현 결과 디자인 폴리시 리뷰" 행 참조.
...
> ⚠ **승인 문서(ADR/계획)를 근거로 옵션을 낼 때, 그 문서 내부 모순을 먼저 확인한다.** "ADR이 X라 했으니 X"는 근거가 아니라 **검증 대상**이다 — 한 문서 안의 **원칙 ↔ 결정문 ↔ 기각한 대안**이 서로 모순되지 않나 대조한다(문서 간 정합은 보면서 문서 내부 정합은 아무도 안 보는 갭). 모순이면 그 조항을 코드에 박기 전에 사용자에게 짚는다. 근거: ADR-0009가 원칙("ff-only 비파괴")과 결정문("상태 동기화"=기각한 양방향 동기화의 상태필드 한정판)이 자기모순인데 설계패널 3R·7.7·구현이 전부 무비판 전제로 깔아, orchestrator가 모순 조항을 복원할 뻔함(사용자가 "이관 vs 동기화" 개념 짚어 발각).
...
- **기술·백엔드 내부 결정도 엔드유저 증상으로 번역한다 (코드 심볼 덤프 금지)**: SHA 검증·DTO 필드·diff 소스·클래스/메서드명 같은 내부 메커니즘을 그대로 나열하지 마라 — 사용자는 그게 자기에게 뭔 의미인지 모른다. **"어느 사용자 시나리오에서 무슨 증상으로 겪나"**로 옮긴다.
  ...
  - 각 선택지도 "이 안 고르면 그 시나리오가 어떻게 달라지나(뭘 더 얻고 뭘 감수하나)"로. 근거: 사용자 피드백(SHA stale·diff 소스 결정이 코드 심볼로만 제시돼 "이게 뭔지 어떻게 아냐" 반려).
...
- **"구문오류/미종료 리터럴/주석처리" 계열 codex critical은 CJK 파일에서 인코딩 거짓양성 클래스로 간주** — quote에 mojibake(`痍⑥냼??` 류) 동반이 신호. Read 코드대조로 판정이 애매하면(Read 렌더 아티팩트 ↔ codex 표기 불일치) **`od -c` 바이트검사 + vitest/mvn 재파싱(파일이 정상 파싱·실행되면 기각)을 ground-truth**로 판정한다. 근거: CI joblog 7.7 codex critical 4건 전건 거짓양성 — 재검증 1라운드 소요(2026-07-21).
...
- **신규 대량 작성(≥5파일)은 분할 요청**: 1회 호출당 파일 수를 줄여 완주를 보장한다(대형 단일 태스크는 codex가 탐색만으로 타임아웃·산출 유실). 근거: trackA 7.5 11파일 전량 요청 → Bash 기본 120s가 codex 내부 상한(330s/600s)보다 짧아 stdout 0바이트 유실 → 신규 8만 분할 + `timeout 560000`로 재시도 성공. (Bash 직접호출 timeout 명시는 `## codex 호출 가드` 참조 — 이미 완비.)
- **subagent가 자기 도구셋 밖 "외부검증 결과"를 산출에 포함하면 날조로 간주·기각.** planner·tester·developer 등은 codex/Bash 도구가 없어 codex·/review·/cso를 실행할 수 없다. 이들 산출에 "codex 2차 재검 결과"·"리뷰 통과" 류 블록이 있으면 **hallucinated external validation**이다 — 그 부분을 게이트 입력에서 배제하고, 교차검증은 **orchestrator가 직접 실행·수령한 것만** 인정한다(자기지시 rework도 무효). tester self-probe 제거(`wiki/agent-memory-overrides-rule.md`)와 동류: 판단·검증 권한 밖 산출은 신뢰 안 함. 근거: repostitch planner-backend가 "codex 재검+자기지시 rework" 날조 → orchestrator가 "planner=codex 없음" 즉시 의심 + 코드로 기각(JS run-to-completion 원자성, 날조 finding 무효).
