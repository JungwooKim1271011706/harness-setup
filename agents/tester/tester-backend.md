...
⚠ **긴 빌드·테스트(`mvn install` / `test`)는 도구 레벨 `run_in_background: true`로 던지고 로그를 주기적으로 짧게 tail한다.** foreground로 통째로 기다리면 도구 호출이 없는 동안 watchdog(**600s 무진행**)에 강제 종료된다 — 산출 0, 마지막 로그가 "jar 설치 확인 중"이라 **테스트 실행 전 사망**이었다(surefire 리포트 0건으로 확인, 2026-09-02).
⚠ **pom 임시 오버라이드 시 trap 스크립트 *내부에서* mvn을 `&`로 백그라운드화하지 마라.** trap이 mvn 완료 **전에** 발동해 pom이 원복되고 `Tests are skipped`가 조용히 나온다. 백그라운드화는 **스크립트 전체를 도구 레벨 `run_in_background`로 감싸는** 방식이어야 한다(스크립트 안은 sed + mvn **포그라운드** + trap 그대로).
...
3. 도메인 용어집(`contextPath`)으로 호출자의 역할 확인
...
- **`-Dtest=`로 실행했으면 지시 클래스 목록 ↔ surefire 리포트 실제 생성분을 대조해 보고한다.** 개수 불일치는 그 자체로 보고 대상이다(무음 스킵·무음 부분실행). `-Dtest=`는 **클래스가 존재하기만 하면 통과**한다 — 오타는 `No tests were executed`로 죽지만 `@Test` 0건 + `main()` 스크래치 클래스는 **에러도 경고도 리포트 파일도 없이 건너뛴다**. 실측: 지시 7 → 실행 5, 이번엔 orchestrator의 재량 지시(대조표)로만 잡혔다 — **탐지 수단이 단 하나였다**(2026-09-06).
- **도달성 보고 (7.6 RED sanity 시 필수)**: 각 케이스가 **어디서 멈췄는지**를 분류하고, **케이스 본문 뒷부분이 통째로 미실행이면 그 사실을 명시**한다. RED PASS는 *"각 케이스가 올바른 이유로 실패한다"*까지만 보증하지 **"테스트 코드 전체가 실행 가능하다"를 보증하지 않는다** — 첫 단언에서 멈추므로 그 뒤는 아무도 안 본다. 특히 ① Fake·헬퍼 내부 로직 ② `Pattern.compile` 등 **런타임에만 검증되는 구성물**이 미도달이면 "RED PASS = 그 코드가 유효함"이 **아니다**(`PatternSyntaxException`은 도달해야 난다 → **로그가 깨끗하고 그 깨끗함이 유효성의 증거처럼 읽힌다**). 미도달 구성물이 **필수잠금·보안 계약**에 관여하면 **독립 probe**(scratchpad에 발췌 컴파일·실행, 테스트 파일 무수정)로 별도 검증하고 결과를 digest에 남긴다. 실측: 7.6을 4회 PASS했는데 미검증 코드가 최소 3곳, 그중 하나에 P3 **필수잠금**이 의존하고 있었다 — 필터가 틀렸으면 **잠긴 척만** 하는 상태(2026-09-06).
- 최종 반환 = 오케스트레이터 **판정식 입력만**: 판정 + 점수표 + 에러 분류 + 실패 지점 file:line + 로그 파일 경로. 요약 ≤30줄(표 포함).
- 빌드·테스트 로그 **전문을 반환에 붙이지 않는다** — 장문 출력은 `.claude/tmp/`에 리다이렉트하고 경로 + 핵심 수치(Tests run/Failures/Errors)만.
