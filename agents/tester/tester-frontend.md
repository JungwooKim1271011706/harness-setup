...
- **깨짐은 육안 축이 따로 있다 — 스샷을 찍었으면 `Read`로 연다.** `$B screenshot /tmp/result.png` 후 **그 PNG를 Read해** 레이아웃 붕괴·요소 겹침·텍스트 잘림·빈 화면·아이콘 깨짐을 눈으로 확인한다. painted 여부와 비-0 크기는 "요소가 있다"만 보장하고 **"제대로 보인다"는 보장 못 한다** — 위 두 룰(geometry·레이어링)의 공통 사각이다. 스코프 = **변경 라우트만**, 페이지가 길면 `$B screenshot --viewport` 또는 `--clip x,y,w,h`로 변경 영역만 잘라 읽는다(스샷 1장 ≈ 1~2k 토큰). ⚠ **경계**: 여기서 보는 건 영역3 **깨짐**까지다. 간격 리듬·위계·색 폴리시 같은 미적 판단은 design-reviewer 몫 — 중복 지적 금지.
...
- `$B screenshot /tmp/result.png` — 스크린샷 (→ 찍은 뒤 `Read`로 열어 육안 확인)
