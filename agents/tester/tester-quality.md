...
- ① **테스트 간 논리모순** — 동일 setup/입력에 상충하는 기대(특히 `@Nested`/형제 케이스가 같은 mock 입력에 서로 다른 행위를 기대). 발견 시 어느 케이스가 계획서 계약과 어긋나는지 명시한다.
...
|----------|-----------|-------------|
| **critical** | 게이트 차단 | tautology, 7c 합의 케이스 누락, 구현 디테일/private 테스트, 가짜 RED 위장, mutation 관점 무력(동작 안 잡음), RED 픽스처 구조 결함(기준9: 테스트 간 논리모순·strict stub 겹침·primitive 매처 NPE·신규 describe 격리 누출) |
