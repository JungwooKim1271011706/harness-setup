...
- **오라클 출처 점검 (기대값이 명세에서 왔나, 구현을 베꼈나)**: 핵심 단언마다 기대값의 출처가 **명세**(feature 문서 요구사항 / co-plan 시나리오 / 7c 합의 케이스 / WI)인지 확인한다. 출처를 못 대고 **구현·stub의 현재 반환을 관측해 베낀 것**이면 그 단언은 무효다 — 구현이 틀려도 영원히 GREEN인 공허 테스트가 된다(oracle = 명세, 구현 독립).
  - **표면은 재작성 LOOP다.** 최초 7.5 작성 시점엔 greenfield prod stub이 아직 없어(순서: 7.5 작성 → stub → 7.6) 베낄 대상이 없다. 그러나 7.6/7.7 FAIL로 **재작성**할 때는 stub·구현이 이미 존재하고 작성자가 그걸 본다 — "stub이 null 반환하니 `assertNull`" 류가 여기서 들어온다. **LOOP≥1 산출은 이 항목을 특히 본다.**
  - 판정: 명세 앵커를 못 대는 핵심 단언 = **critical**(공허 테스트). 부수 단언이면 major.
  - 작성자≠구현자 분리가 절반을 막지만, 재작성 루프에서 작성자가 stub/prod를 본 경우의 잔여 구멍은 이 명세-앵커 점검으로만 닫힌다.
...
|----------|-----------|-------------|
| **critical** | 게이트 차단 | tautology, **핵심 단언의 오라클 출처 부재(기준4 — 구현·stub 관측 베낌)**, 7c 합의 케이스 누락, 구현 디테일/private 테스트, 가짜 RED 위장, mutation 관점 무력(동작 안 잡음), RED 픽스처 구조 결함(기준9: 테스트 간 논리모순·strict stub 겹침·primitive 매처 NPE·신규 describe 격리 누출·DB픽스처 런타임계약·오라클 변별력) |
...
- 기준2(FIRST): [점검 결과]
- 기준3(행위 기준): [점검 결과]
- 기준4(Assertion 품질 + **오라클 출처**): [tautology 점검 + 핵심 단언의 기대값이 어느 명세(feature 문서/7c 케이스/WI) 줄에서 왔는지. LOOP≥1 산출이면 특히]
- ...
