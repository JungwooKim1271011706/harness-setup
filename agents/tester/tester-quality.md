...
- **작성자가 보고한 케이스 수·커버리지 매핑을 신뢰하지 말고 직접 세어 대조한다. 불일치는 critical.** 이 게이트의 루브릭은 *"작성된 테스트의 품질"*(공허 단언·매처·stub)을 보는데, 실제로 새는 것은 **"작성됐다는 주장 자체의 참/거짓"**이라 축이 다르다. 실측 3건: ① R21 반증이 **산문으로만** 충족(목 설정이 그 분기에 도달하지 않아 단언이 실행 안 됨 — 서술은 정확했고 코드가 서술을 구현하지 않았다) ② 재작성한 기대값이 **구현 전 값과 우연히 동일**(수정 안 된 것과 구분 불가) ③ 갭 종결 주장이 **존재하지 않는 테스트를 인용**(`A3b`가 없었다). 세 건 다 규칙이 아니라 **임기응변**으로 잡혔다(2026-08-24).
- **"X가 이 갭을 커버한다"는 종결 주장은 X를 grep해 실재부터 확인한다.** 존재 확인 1회 grep ↔ 미확인 시 라운드 1회. 이건 편집도 품질도 아니라 **기존 어느 그물에도 안 걸린다**(`## 산출 수신 계약`은 digest를 신뢰하고, 디스크 반영 검증은 *편집 위임 후*만 커버한다).
- **각 케이스가 주장하는 갭에 실제로 실행 도달하는가**를 본다 — 목 설정이 그 분기에 닿는지. 산문 주장과 실행 경로의 분리를 명시 점검한다(도달 못 하면 단언이 있어도 검증력 0).
