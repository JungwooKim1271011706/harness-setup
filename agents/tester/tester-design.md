...
- **(R23) 주석 안에서 `*` 바로 뒤 `/` 금지** — 케이스 이름을 glob으로 나열하는 표기(`ET-*/ETD-*`)가 자연스럽지만 Java·JS 블록주석 안에서는 **종료 토큰 `*/`를 형성**한다. 주석이 문장 중간에서 끝나고 뒤 텍스트가 코드로 파싱된다. 글롭 나열은 **쉼표 구분**으로 쓴다. 자가검사: 주석 블록 안에서 `[^ *]\*/` 패턴 탐색. 근거: javadoc 한 줄이 **36개 구문에러** → `test-compile` BUILD FAILURE → 지시된 **18개 클래스 전부 실행 불가**(2026-07-30, `javac -encoding UTF-8` 단독 재현으로 인코딩 문제 아님을 확정 후 1라운드 복구).
