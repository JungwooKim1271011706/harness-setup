...
- **(R22) 제어문자를 리터럴로 타이핑하지 않는다 — 런타임 조립만.** 제어문자가 필요한 케이스(CWE-117 로그 인젝션 위생처리 검증 등)는 **char 코드값 런타임 조립**만 쓴다: `String.valueOf((char) 7)`. 이스케이프 표기를 소스·주석에 직접 적으면 도구를 경유하며 **실제 바이트로 기록될 수 있다**(실측). 설명이 필요하면 "제어문자(BEL, 코드 7)"처럼 **말로 푼다**. **작성 후 자가검증 필수** — `Grep` 패턴 `[\x00-\x08\x0B\x0C\x0E-\x1F]`(패턴 자체는 정규식이라 안전)로 산출 파일을 확인하고 결과를 반환에 포함한다. ⚠ 오염되면 ripgrep이 그 파일을 **binary로 무음 제외**해 이후 모든 인벤토리 grep이 조용히 실패한다 — `file`은 "UTF-8 text"로 보고하는데 `grep`만 binary로 판정해 **자각 없이 지나간다**. 근거: 7a 산출이 계획서(SSOT)에 raw 제어바이트를 주입했고 자가신고("3개")와 실측(4바이트 = NUL 2 + BEL 2)이 불일치, 봉인도 불완전했다. 방지 지시를 주입한 뒤 후속 5배치는 **전부 오염 0** — 지시가 있으면 안 밟는다(2026-08-06). 부가: **대용량 단일 Write 지양**(오염이 대량 append에서 발생) — 뼈대 Write → 케이스별 Edit append.
...
- **(R24) 테스트 인프라 코드(경로 해석·픽스처 로딩·공통 헬퍼)는 실패 반경을 최소화한다.** 케이스 설계는 7a∥7b로 2소스 교차검증되지만 그걸 코드로 옮길 때의 **하네스 코드는 검증 축 밖**이라 단일 실패점이 된다.
  - **CWD 독립 경로 해석에 `getAbsoluteFile().getParentFile()`을 쓰지 마라.** `getParentFile()`은 파일시스템에 접근하지 않고 **문자열만 파싱**해 트레일링 `.`만 떼어낸다 → `<repo>/mod/.`의 부모가 **자기 자신**이 된다. 상대경로 후보를 직접 `isDirectory()`로 시험하라(실제 FS 호출이라 OS가 `.`/`..`를 해석한다).
  - **`static` 필드 초기화에 파일시스템·네트워크 접근을 두지 마라.** 실패가 `ExceptionInInitializerError` → 나머지 케이스 `NoClassDefFoundError`로 **클래스 전체에 전이**된다. lazy로 잡거나 최소한 그 반경을 인지하고 쓴다.
  - **한 스위트 안에서 같은 문제를 두 방식으로 풀지 마라.** 이미 있는 헬퍼 패턴을 **먼저 grep**한다.
  - ⚠ **CWD 의존은 조용하다** — repo 루트에서 실행하면 파일시스템이 `.`를 해석해 **우연히 통과**한다. 실행 위치가 하나만 달랐어도 GREEN 단계까지 잠복했을 것이다. 근거: 두 파일이 공유하는 `repoRoot()` 헬퍼 **하나**가 케이스 **24건을 한꺼번에 죽였고**, 같은 스위트 안에 올바른 구현(`isDirectory()` 시험)이 **이미 있었다** — 한 작성 세션이 같은 문제를 두 방식으로 풀고 한쪽만 틀렸다(2026-09-06).
...
- **RED가 아니라 이미 PASS할 것으로 보이는 케이스를 지목한다 (필수).** 케이스명 + 성격을 (a)정당한 negative 대조군 / (b)회귀가드 중 무엇으로 의도했는지 1줄씩. 7.6이 이 목록을 받아 3분류하고 **(c)tautological이 1건이라도 있으면 불통과**시킨다(`docs/playbook-tdd.md` 7.6 "currently-PASS 케이스 3분류"). 지목을 빠뜨리면 7.6이 그 축을 못 보고 tautological이 GREEN 이후까지 간다.
