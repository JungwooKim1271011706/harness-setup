...
> **UI 태그 시 화면 규모 명시(필수)**: 변경이 **신규 화면/큰 레이아웃 변경**인지 **기존 화면 소소한 수정**(문구·버튼 1개 등)인지 한 줄로 표시한다. 예: `## 화면 규모: 신규 화면` 또는 `## 화면 규모: 기존 소소수정`. orchestrator 디자인 목업 게이트가 이 표시로 발동을 판단한다.
...
|---|---|---|
| (예) 신규 목록화면 JSP | `.../userList.jsp` | 같은 모듈 최근 목록화면, 페이징·검색폼 구조 동일 |
